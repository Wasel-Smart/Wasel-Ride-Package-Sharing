...
export default function PaymentMethodsScreen() {
  ...
  const handleAddPayment = () => {
    Alert.alert('إضافة وسيلة دفع', 'اربط Stripe أو مزود دفع ثاني');
  };

  const handleSetPrimary = (_id: string) => {
    Alert.alert('تم بنجاح', 'تم تحديث وسيلة الدفع الأساسية');
  };
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>إدارة خيارات الدفع</Text>
      </View>

      {error && (
        <View style={styles.errorBanner}>
          <Text style={styles.errorText}>فشل تحميل وسائل الدفع</Text>
        ...
        <View key={method.id} style={styles.methodCard}>
          <View style={styles.methodInfo}>
            ...
            <View style={styles.methodDetails}>
              ...
              {method.type === 'wallet' && (
                <Text style={styles.balance}>الرصيد: JOD {(method.balance ?? 0).toFixed(2)}</Text>
              )}
              {method.primary && (
                <View style={[styles.primaryBadge, { backgroundColor: colors.teal + '18' }]}>
                  <Text style={[styles.primaryBadgeText, { color: colors.teal }]}>أساسية</Text>
                </View>
              )}
            </View>
          </View>
          ...
          {!method.primary && (
            <TouchableOpacity onPress={() => handleSetPrimary(method.id)}>
              <Text style={[styles.setPrimaryText, { color: colors.teal }]}>تعيين كأساسية</Text>
            </TouchableOpacity>
          )}
        </View>
      ))}
      ...
      >
        <Text style={styles.addButtonText}>+ إضافة وسيلة دفع</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}

    ...
    <ScreenShell testID="advanced-search-screen">
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.statusRow}>
          <StatusPill
            label={`${filteredRides.length} مشاوير متاحة`}
            tone={colors.green}
            icon="search"
          />
          <StatusPill
            label={`الترتيب حسب ${SORT_LABELS[sortBy]}`}
            tone={colors.blue}
            icon="funnel"
          />
        </View>
        ...
        <SectionHeader
          eyebrow="بحث ذكي"
          title="دور على المشوار الأنسب إلك"
          body="فلتر حسب السعر والتقييم والمقاعد وغيرها لتلاقي أفضل خيار."
        />
        ...
        <View style={styles.filterHeader}>
          <PrimaryButton
            label={showFilters ? 'إخفاء الفلاتر' : `إظهار الفلاتر${activeFiltersCount > 0 ? ` (${activeFiltersCount})` : ''}`}
        ...
        {showFilters && (
          <PremiumPanel style={styles.filtersPanel}>
            <SectionHeader
              eyebrow="الفلاتر"
              title="ضيّق البحث"
              body="استخدم الفلاتر لتقليل النتائج"
            />

            {/* Sort Options */}
            <Text style={styles.filterLabel}>رتب حسب</Text>
            ...
            {/* Price Range */}
            <Text style={styles.filterLabel}>نطاق السعر: {priceRange[0]} - {priceRange[1]} JOD</Text>
            ...
            {/* Min Seats */}
            <Text style={styles.filterLabel}>أقل عدد مقاعد: {minSeats}</Text>
            ...
            <View style={styles.toggles}>
              <FilterToggle
                label="السائقون الموثقون فقط"
                active={verifiedOnly}
                onToggle={() => setVerifiedOnly(!verifiedOnly)}
                icon="shield-checkmark"
              />
              <FilterToggle
                label="الحجز الفوري فقط"
        ...
        <View style={styles.metrics}>
          <MetricTile label="المتاح" value={String(filteredRides.length)} tone={colors.teal} />
          <MetricTile label="متوسط السعر" value={`${(filteredRides.reduce((sum, r) => sum + r.priceJod, 0) / filteredRides.length || 0).toFixed(1)} JOD`} tone={colors.gold} />
        </View>

        {filteredRides.length === 0 ? (
          <StateNotice
            icon="search"
            title="ما لقينا مشاوير"
            body="جرّب تعدّل الفلاتر أو كلمات البحث"
            tone={colors.amber}
          />
        ) : (
        ...
        <InfoCard
          icon="flash"
          title="حجز فوري"
          body="المشاوير ذات الحجز الفوري بتتأكد مباشرة بدون انتظار موافقة السائق."
          tone={colors.blue}
        />
      </ScrollView>
    </ScreenShell>
    ...
    <PremiumPanel style={styles.rideCard}>
        ...
        <View style={styles.priceContainer}>
          ...
          {ride.instantBook && (
            <View style={styles.instantBadge}>
              ...
              <Text style={styles.instantText}>فوري</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={styles.rideDetails}>
        ...
        <View style={styles.rideDetail}>
          ...
          <Text style={styles.rideDetailText}>{ride.seatsAvailable} مقاعد</Text>
        </View>
        <View style={styles.rideDetail}>

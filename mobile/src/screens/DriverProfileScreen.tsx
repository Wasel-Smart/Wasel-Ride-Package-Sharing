...
export default function DriverProfileScreen({ route }: { route: { params: { driverId: string } } }) {
  ...
  const { data: driver, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!driverId) throw new Error('رقم السائق غير موجود');
      const response = await apiClient.get<Driver>(`drivers/${driverId}`);
      if (!response.data || response.error) throw new Error(response.error || 'السائق غير موجود');
  ...
  if (error || !driver) {
    return (
      <ScrollView style={styles.container}>
        <View style={styles.header}>
          <View style={styles.avatarPlaceholder} />
          <Text style={styles.name}>السائق غير موجود</Text>
          <Text style={styles.errorText}>ما قدرنا نحمّل ملف السائق. جرّب مرة ثانية.</Text>
        </View>
      </ScrollView>
    );
  }
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        ...
        <View style={styles.ratingContainer}>
          ...
          <Text style={styles.trips}>{driver.totalTrips.toLocaleString()} مشاوير</Text>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.infoRow}>
          <Text style={styles.label}>الشركة والموديل:</Text>
        ...
        <View style={styles.infoRow}>
          <Text style={styles.label}>اللوحة:</Text>
          <Text style={styles.value}>{driver.vehicle.plate}</Text>
        </View>
      </View>

      <View style={styles.section}>
        <Text style={styles.sectionTitle}>تفاصيل السائق</Text>
        <View style={styles.infoRow}>
          <Text style={styles.label}>سنوات النشاط:</Text>

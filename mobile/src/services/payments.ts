...
import { toMinorUnits } from '../../src/shared/currency/currency';
...
class PaymentService {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('stripe-payments-v2', {
        body: {
          ...
          metadata: { purpose: 'wallet_top_up', user_id: userId },
          idempotency_key: `wallet-top-up:${userId}:${toMinorUnits(amount, currency)}`,
        },
        headers: {
      ...
      if (data?.clientSecret && data?.paymentIntentId) {
        return { success: true, paymentId: data.paymentIntentId, clientSecret: data.clientSecret };
      }
  ...
  async withdrawFunds(userId: string, amount: number): Promise<PaymentResult> {
    try {
      ...
      return { success: false, error: 'Withdrawals require an approved server-side payout workflow.' };
    } catch (error) {

...
function createBookingParticipantQuery(result: { data: unknown; error?: unknown }) {
  const maybeSingle = jest.fn().mockResolvedValue(result);
  const passengerEq = jest.fn().mockReturnValue({ maybeSingle });
  const tripEq = jest.fn().mockReturnValue({ eq: passengerEq });
  const select = jest.fn().mockReturnValue({ eq: tripEq });

  return { select, tripEq, passengerEq, maybeSingle };
}
...
describe('MobileChatService', () => {
  ...
  it('sends an authenticated passenger message through the messages table', async () => {
    const tripQuery = createMaybeSingleQuery({ data: { driver_id: 'driver-1' } });
    const bookingQuery = createBookingParticipantQuery({ data: { passenger_id: 'user-1' } });

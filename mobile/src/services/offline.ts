...
export class OfflineService {
  ...
  constructor() {
    void this.initNetworkMonitoring().catch(err =>
      console.error('[Offline] Network monitoring failed:', err),
    );
  }
  ...
  subscribeToStats(cb: (stats: OfflineStats) => void): () => void {
    ...
    void this.getStats().then(cb).catch(console.error);
  ...
  async queueOfflineAction(
    ...
    if (persistedDedup.has(dedupKey)) {
      console.log('[Offline] Duplicate action skipped (persisted):', action.type);
      return;
    }
    ...
    void this.notifyStats();
    console.log(`[Offline] Queued: ${action.type} (id=${offlineAction.id})`);
  }
    ...
    try {
      ...
      results.forEach((result, i) => {
        ...
        if (result.status === 'fulfilled') {
          console.log(`[Offline] Synced: ${action.type} (id=${action.id})`);
        } else {
          console.error(`[Offline] Failed: ${action.type}`, result.reason);
          const retried = incrementOfflineRetry(action, this.maxRetries);
          if (retried) {
            failed.push(retried);
          } else {
            console.error(`[Offline] Discarding after ${this.maxRetries} retries: ${action.id}`);
          }
        }
      });
  ...
  async clearCache(): Promise<void> {
    try {
      ...
      void this.notifyStats();
    } catch (err) { console.error('[Offline] clearCache error:', err); }
  }

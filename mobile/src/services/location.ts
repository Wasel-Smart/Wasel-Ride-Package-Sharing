...
export class LocationTrackingService {
  ...
  async startTracking(
    ...
    this.watchId = Geolocation.watchPosition(
      ...
      err => console.error('[LocationTracking] watchPosition error:', err),
      {
  ...
  private connect(wsUrl: string, token: string): void {
    ...
    this.socket.on('connect_error', (err) => {
      console.warn('[LocationTracking] connect_error:', err.message);
    });
  ...
  private startRestPoll(): void {
    ...
    this.restPollTimer = setInterval(async () => {
      ...
      for (const [key, desc] of this.activeSubs.entries()) {
        ...
        try {
          const token = mobileAuth.getAccessToken();
          const res = await fetch(`${apiUrl}/rides/${encodeURIComponent(tripId)}/live`, {

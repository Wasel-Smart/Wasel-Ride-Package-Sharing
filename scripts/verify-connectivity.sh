...
EDGE_BASE_URL="${VITE_SUPABASE_URL:-}"
if [ -z "$EDGE_BASE_URL" ] && [ -f .env ]; then
  EDGE_BASE_URL=$(grep '^VITE_SUPABASE_URL=' .env | cut -d'=' -f2- | tr -d '"')
fi
EDGE_FUNCTION_NAME="${VITE_EDGE_FUNCTION_NAME:-make-server-0b1f4071}"
EDGE_HEALTH_URL="${EDGE_BASE_URL}/functions/v1/${EDGE_FUNCTION_NAME}/health"

run_check "Edge function health" "test_http '$EDGE_HEALTH_URL'"

# Test edge function CORS
run_check "Edge function CORS" "curl -s -X OPTIONS -H 'Origin: https://wasel14.online' '${EDGE_HEALTH_URL}' | grep -q 'access-control-allow-origin'"

# Test edge function response time
echo -n "  Testing response time... "
START_TIME=$(date +%s%N)
curl -s "${EDGE_HEALTH_URL}" > /dev/null
END_TIME=$(date +%s%N)
...
test_http "${EDGE_BASE_URL}/functions/v1/${EDGE_FUNCTION_NAME}/health" && ENDPOINTS_OK=$((ENDPOINTS_OK + 1))
test_http "${EDGE_BASE_URL}/functions/v1/${EDGE_FUNCTION_NAME}/trust/status" 401 && ENDPOINTS_OK=$((ENDPOINTS_OK + 1))
test_http "${EDGE_BASE_URL}/functions/v1/${EDGE_FUNCTION_NAME}/communications/preferences" 401 && ENDPOINTS_OK=$((ENDPOINTS_OK + 1))

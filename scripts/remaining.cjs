...
function collect(dir,o=[]){for(const e of fs.readdirSync(dir,{withFileTypes:true})){const f=path.join(dir,e.name);if(e.isDirectory()){if(['locales','node_modules','__tests__'].includes(e.name))continue;collect(f,o);}else if(e.isFile()&&/\.(tsx?)$/.test(e.name)&&!/\.(test|spec)\./.test(e.name))o.push(f);}return o;}
const out=[];
for(const f of collect(ROOT)){
  ...
  function walk(n){if(!n)return;
    if(n.kind===ts.SyntaxKind.JsxText){const t=n.text.replace(/\s+/g,' ').trim();if(t&&hasLat(t)&&!hasAr(t)&&!allowed(t))out.push({file:f.replace(ROOT+'\\',''),text:t});}
    else if(n.kind===ts.SyntaxKind.JsxAttribute){const nm=n.name&&n.name.text;if(nm&&VIS.includes(nm)&&n.initializer&&n.initializer.kind===ts.SyntaxKind.StringLiteral){const t=n.initializer.text;if(t&&hasLat(t)&&!hasAr(t)&&!allowed(t))out.push({file:f.replace(ROOT+'\\',''),text:t});}}

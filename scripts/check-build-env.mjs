...
function safeResolve(filePath) {
  ...
  const allowed = path.resolve(PROJECT_CWD);
...
function loadEnvFile(filePath) {
  ...
  try {
    const contents = fs.readFileSync(safePath, 'utf8');
    ...
    // Skip unreadable env files (e.g., OneDrive cloud-only placeholders).
  }
}
...
if (mode === 'production') {
  ...
  if (!appUrl.startsWith('https://')) {
    console.error(`\n❌ Build failed: VITE_APP_URL must use HTTPS in production. Current: "${appUrl}"\n`);
    process.exit(1);
  }

  if (!supabaseUrl) {
    console.error('\n❌ Build failed: VITE_SUPABASE_URL is not set for production build.');
    process.exit(1);
  }

  if (!supabaseUrl.startsWith('https://')) {
    console.error(`\n❌ Build failed: VITE_SUPABASE_URL must use HTTPS in production. Current: "${supabaseUrl}"\n`);
    process.exit(1);
  }
}

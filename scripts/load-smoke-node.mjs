...
function validateUrl(raw) {
  ...
  if (!ALLOWED_BASE_URLS.some(pattern => pattern.test(url.href))) {
    throw new Error(`URL not in allowed targets: ${url.href}`);
  }
  return url.href;
}

const rawTargetUrl = process.argv[2] ?? process.env.BASE_URL ?? 'https://wasel14.online/health';
...
const artifactsDir = resolve(process.cwd(), 'artifacts', 'load');
if (!artifactsDir.startsWith(PROJECT_ROOT)) {
  throw new Error('Path traversal detected: artifactsDir resolves outside project root');
}
...
const reportFile = resolve(artifactsDir, 'node-smoke-report.json');
if (!reportFile.startsWith(PROJECT_ROOT)) {
  throw new Error('Path traversal detected: report file resolves outside project root');
}

...
const ROOT = 'C:/Users/user/OneDrive/Desktop/Wdoubleme';
...
const un = JSON.parse(fs.readFileSync(safeResolve(SCRIPTS_DIR, 'hardcoded-unmatched.json'), 'utf8'));
const files = [...new Set(un.map((r) => r.file))];
let needInjection = 0, alreadyHave = 0;
const needList = [];
for (const f of files) {
  const safeF = safeResolve(ROOT, f);
  const c = fs.readFileSync(safeF, 'utf8');
...
for(const f of collectFiles(SRC_DIR)){
...
fs.writeFileSync(safeResolve(SCRIPTS_DIR, 'mixed-findings.json'), JSON.stringify(uniq,null,2));

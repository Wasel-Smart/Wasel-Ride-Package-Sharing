...
// On Node.js v25 + Windows in non-TTY environments, any ReadStream backed by
// a broken pipe emits UNKNOWN (-4094) as an unhandled error event. Intercept
// it at the uncaughtException level so the dev server keeps running.
// CI=true tells Vite to skip its own stdin listeners (readline shortcuts +
// the SIGTERM 'end' listener).
if (!process.stdin.isTTY) {
  process.env.CI = 'true';
  process.on('uncaughtException', err => {
    if (err.code === 'UNKNOWN' && err.syscall === 'read') return;
    console.error(err);
    process.exit(1);
  });
}
...
async function main() {
  ...
  for (const sig of ['SIGINT', 'SIGTERM']) {
    process.once(sig, () => server.close().finally(() => process.exit(0)));
  }
}

...
const ALLOWED_DOMAINS = [
  'supabase.co', 'supabase.net', 'wasel14.online', 'localhost',
  'google.com', 'googleapis.com', 'accounts.google.com',
  'facebook.com', 'api.stripe.com', 'api.twilio.com',
...
async function fetchWithTimeout(url, options = {}) {
  if (!isValidUrl(url)) {
    throw new Error(`Invalid or unauthorized URL: ${url}`);
  }
...
async function verifyStripe() {
  ...
  requireSourceToken(provider, 'supabase/functions/stripe-payments-v2/index.ts', 'paymentIntents.create');
  requireSourceToken(provider, 'supabase/functions/stripe-payments-v2/index.ts', 'refunds.create');

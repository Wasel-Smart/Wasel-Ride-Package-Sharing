...
async function checkEventDurability() {
  const brokerPath = join(root, 'src', 'platform', 'event-broker.ts');
  const broker = readFileSync(brokerPath, 'utf8');
  const required = ['InMemoryEventBroker', 'SupabaseEventBroker', 'event_outbox'];
  const missing = required.filter(token => !broker.includes(token));

  record(
    'event-broker-durability-contract',
    missing.length === 0 ? 'pass' : 'fail',
    missing.length === 0
      ? 'dual-mode broker (memory + Supabase durable outbox) present'
      : `missing event broker tokens: ${missing.join(', ')}`,
  );
}

async function checkDeploymentTargets() {
  const required = [
    'vercel.json',
    'docker-compose.yml',
  ...
  record(
    ...
    missing.length === 0 ? 'pass' : 'fail',
    missing.length === 0
      ? 'Vercel, Docker local, and Supabase configs present'
      : `missing deployment configs: ${missing.join(', ')}`,
  );
}
...
async function checkDockerfiles() {
  ...
  record(
    ...
    missing.length === 0 ? 'pass' : 'fail',
    missing.length === 0 ? 'Docker local development configs present' : missing.join(', '),
  );
}

async function checkPaymentContracts() {
  const service = readFileSync(
    join(root, 'supabase', 'functions', 'stripe-payments-v2', 'index.ts'),
    'utf8',
  );
  ...
  record(
    ...
    missing.length === 0 ? 'pass' : 'fail',
    missing.length === 0
      ? 'Stripe v2 contracts enforce auth, amount caps, currency allowlist, and idempotency'
      : `missing Stripe contract tokens: ${missing.join(', ')}`,
  );
}
...
await checkEventDurability();
await checkDeploymentTargets();

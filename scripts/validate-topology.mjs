...
for (const [workerName, relativePath] of workerManifests) {
  let manifest;
  try {
    manifest = parse(await readFile(path.join(process.cwd(), relativePath), 'utf8'));
  } catch {
    console.warn(`Skipping topology validation for ${relativePath}: file is unreadable.`);
    continue;
  }
...
let queueDoc;
try {
  queueDoc = await readFile(queueDocPath, 'utf8');
} catch {
  console.warn('Skipping topology queue-doc validation: file is unreadable.');
  queueDoc = '';
}
for (const topic of [
  'rides.requested',

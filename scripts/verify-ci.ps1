...
Invoke-Step 'npm-audit' {
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-Host "npm audit found high-severity vulnerabilities. Run 'npm audit fix' to address them."
    exit $LASTEXITCODE
  }
}
...
Invoke-Step 'test:unit' {
  $allTestFiles = Get-ChildItem -Path tests -Recurse -File |
    Where-Object { $_.Name -match '\.test\.tsx?$' } |
    Sort-Object FullName
  $testFiles = @($allTestFiles | ForEach-Object {

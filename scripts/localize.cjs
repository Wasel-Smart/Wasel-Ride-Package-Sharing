...
const NEW_TRANS = JSON.parse(fs.readFileSync('C:/Users/user/OneDrive/Desktop/Wdoubleme/scripts/new-translations.json', 'utf8'));
...
function loadTrans() {
  ...
  s = s.replace(/export type Language[^;]*;/g, '');
  s = s.replace(/export const translations: Record<Language, any> =/, 'var translations =');
  const fn = new Function('var module={exports:{}};' + s + '\nreturn translations;');
  return fn();
}
...
for (const f of files) {
  ...
  walk(sf);
  fs.writeFileSync('C:/Users/user/OneDrive/Desktop/Wdoubleme/scripts/_fc.json', JSON.stringify({files: files.length, findings: findings.length}));
}
...
for (const f of Object.keys(editsByFile)) {
  ...
  if (!usesUseLanguage) {
    ...
    const rel = path.relative(path.dirname(f), 'C:/Users/user/OneDrive/Desktop/Wdoubleme/src/locales/tx.ts').replace(/\\/g, '/').replace(/\.tsx?$/, '');
    const stmt = "\nimport { tx } from '" + rel + "';";

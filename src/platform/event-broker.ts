  ...
  private deliver(message: BrokerMessage): void {
    this.listeners.get(message.topic)?.forEach(handler => {
      void Promise.resolve(handler(message)).catch(err =>
        console.error('[broker] handler error', message.topic, err),
      );
    });
    this.anyListeners.forEach(handler => {
      void Promise.resolve(handler(message)).catch(err =>
        console.error('[broker] any-handler error', err),
      );
    });
  }
...
class SupabaseEventBroker implements EventBroker {
  ...
  private async persist(message: BrokerMessage): Promise<void> {
    if (this.proxyAvailable) {
      ...
      if (!result.ok) {
        console.warn(
          '[broker] proxy publish failed, falling back to direct Supabase',
          result.error,
        );
  ...
  private async persistDirect(message: BrokerMessage): Promise<void> {
    try {
      ...
      if (error) {
        console.error('[broker] failed to persist event', message.topic, error.message);
      }
    } catch (err) {
      console.error('[broker] persist threw', message.topic, err);
    }
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      this.channel = this.client
        .channel(`outbox:${Math.random().toString(36).slice(2, 8)}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: OUTBOX_TABLE }, () => {
          void this.processPending();
        })
        .subscribe();
    } catch (err) {
      console.warn('[broker] realtime subscribe failed, relying on poll', err);
    }
    ...
    try {
      ...
      if (!this.proxyAvailable) {
        const { data, error } = await this.client
          ...
          .limit(BATCH_SIZE);
        if (error) {
          console.warn('[broker] poll failed', error.message);
          return;
        }
      ...
      for (const row of rows) {
        ...
        try {
          ...
          console.error('[broker] handler error for', message.topic, deliverErr);
        }
        ...
        if (delivered) {
          if (this.proxyAvailable) {
            ...
            await this.client
              ...
              .then(({ error: updateErr }) => {
                if (updateErr) {
                  console.warn('[broker] failed to mark processed', message.id, updateErr.message);
                }
              });
          }
        } else {
          ...
          if (this.proxyAvailable) {
            ...
            await this.client
              ...
              .then(({ error: updateErr }) => {
                if (updateErr) {
                  console.warn('[broker] failed to update attempts', message.id, updateErr.message);
                }
              });
          }
        }
      ...
      console.error('[broker] processPending error', err);
    } finally {

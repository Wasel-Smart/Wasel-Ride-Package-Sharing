...
async function proxyWriteDeadLetter(dlqPayload: Record<string, unknown>): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('[dlq] proxy write failed', await response.text().catch(() => 'unknown'));
      return false;
    }

    return true;
  } catch (err) {
    console.warn('[dlq] proxy write threw', err);
    return false;
  }
}
...
export abstract class BaseWorker<T = unknown> {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      createStructuredLogEntry('info', `Worker ${this.config.name} started`, this.config.name, {
    ...
    if (isSupabaseConfigured && defaultSupabase) {
      void Promise.resolve(defaultSupabase.from('dead_letter_messages').insert(dlqPayload))
        .then(({ error: insertError }) => {
          if (insertError) {
            console.error('[dlq] failed to persist dead letter', insertError.message);
          }
        })
        .catch((dlqError: unknown) => {
          console.error('[dlq] persist threw', dlqError);
        });
    }
  }
}

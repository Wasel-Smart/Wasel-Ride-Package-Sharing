...
export async function decryptData(encryptedData: string): Promise<string> {
  try {
    ...
    // Decryption failed — rethrow with a safe message
    ...
    try {
      if (typeof localStorage.length === 'number' && typeof localStorage.key === 'function') {
        for (let i = localStorage.length - 1; i >= 0; i--) {
          ...
          if (!key) continue;
          if (key.startsWith('secure_')) localStorage.removeItem(key);
        }
        return;
      }
    } catch {
      // fallback to enumeration below
    }

    // Fallback for environments where `localStorage.key` or `length` are not standard
    const storage = localStorage as unknown as Record<string, string>;
    for (const k in storage) {
      try {
        if (!Object.prototype.hasOwnProperty.call(localStorage, k)) continue;
        if (typeof k === 'string' && k.startsWith('secure_')) {
          try {
            localStorage.removeItem(k);
          } catch {
            // If removeItem isn't available, try deleting the property directly
            try {
              delete storage[k];
            } catch {
              // best-effort delete

/**
 * CSRF (Cross-Site Request Forgery) — client-side defense-in-depth
 *
 * NOTE: This SPA authenticates with a bearer token (Supabase JWT), not cookie
 * sessions, so a token stored in sessionStorage + sent as `X-CSRF-Token` is NOT
 * a real cross-site defense — the server has no cookie to compare it against and
 * CORS + SameSite handling is the authoritative boundary. This module instead
 * guards against same-tab double-submits / replay within the client. The real
 * request boundary (origin/CORS checks, auth, server-side rate limiting) is
 * enforced in the edge functions under supabase/functions/_shared/request-security.ts.
 */
...
export function addCSRFHeader(headers: HeadersInit = {}): HeadersInit {
  ...
  if (headers instanceof Headers) {
    ...
    copy.set(CSRF_TOKEN_HEADER, token);
    return copy;
  }

  if (Array.isArray(headers)) {
    return [...headers, [CSRF_TOKEN_HEADER, token]];
  }

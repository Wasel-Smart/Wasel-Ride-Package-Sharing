...
function createToastElement(options: ToastOptions): HTMLDivElement {
  const { message, type = 'info', action, dismissible = true, icon } = options;
...
export function showToast(options: ToastOptions): string {
  const { duration = 5000, position = 'bottom' } = options;

  const toastId = `toast-${Date.now()}-${crypto.randomUUID().split('-')[0]}`;

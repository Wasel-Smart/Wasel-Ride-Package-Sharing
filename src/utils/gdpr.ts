...
class GDPRCompliance {
  ...
  async requestDataExport(userId: string): Promise<DataExportRequest> {
    try {
      ...
      this.generateDataExport(userId).catch(error => {
        logger.error('Data export generation failed', error, { userId: sanitizeLogMessage(userId) });
      });

      return request;
    } catch (error) {
      logger.error('Failed to request data export', error, { userId: sanitizeLogMessage(userId) });
      throw error; // Re-throw to allow upstream handling
    }
  }
  ...
  private async generateDataExport(userId: string): Promise<string> {
    try {
      ...
      // Upload the export to Supabase Storage so the user can download it
      // via a signed URL rather than an inline data: URI.
      await supabase.storage
        .from('gdpr-exports')
        .upload(downloadUrl, new Blob([exportJson], { type: 'application/json' }), {
          upsert: true,
          contentType: 'application/json',
        });

      logger.info('Data export generated', { userId: sanitizeLogMessage(userId) });

      return downloadUrl;
    } catch (error) {
      logger.error('Failed to generate data export', error, { userId: sanitizeLogMessage(userId) });
      throw error; // Re-throw to allow upstream handling
    }
  }
  ...
  async requestDeletion(userId: string, reason?: string): Promise<DataDeletionRequest> {
    try {
      ...
      logger.error('Failed to request deletion', error, { userId: sanitizeLogMessage(userId) });
      throw error; // Re-throw to allow upstream handling
    }
  }
  ...
  async cancelDeletion(userId: string): Promise<void> {
    try {
      ...
      logger.error('Failed to cancel deletion', error, { userId: sanitizeLogMessage(userId) });
      throw error; // Re-throw to allow upstream handling
    }
  }
    ...
    try {
      ...
      for (const request of requests) {
        try {
          ...
          logger.info('User data deleted', { userId: sanitizeLogMessage(request.user_id) });
        } catch (error) {
          logger.error('Failed to delete user data', error, { userId: sanitizeLogMessage(request.user_id) });
        }
      }
    } catch (error) {

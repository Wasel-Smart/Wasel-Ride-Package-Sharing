...
function calculateProofScore(record: CorridorBetaMetricRecord) {
  ...
  const baseDemandScore = Math.min(
    10,
    (record.completedRides / Math.max(1, WEEKLY_RIDE_GOAL)) * 10,
  );

  return Math.round(
    Math.min(100, weeklyScore + repeatScore + supplyScore + consistencyScore + baseDemandScore),
  );
}

...
export async function getDirectProfile(userId: string): Promise<RawProfile | null> {
  ...
  const rating = toNumber(context.user.rating_as_driver ?? context.user.rating, 5);
  return mapProfileFromContext(context, { tripCount, rating });
}
...
export async function updateDirectProfile(userId: string, updates: Record<string, unknown>) {
  // For new users the canonical row may not exist yet. Seed it from auth metadata
  // so ensureCanonicalUser can create it, then proceed with the update.
  let context;
  try {
    ...
    // If the canonical user row still can’t be created (e.g. missing required
    // fields), surface a clear message rather than a generic DB error.
    const message = seedError instanceof Error ? seedError.message : String(seedError);
    if (message.includes('incomplete') || message.includes('required')) {
      throw new Error(
        'Your profile is missing required information. Please complete your account setup.',
        { cause: seedError },
      );
    }
    throw seedError;

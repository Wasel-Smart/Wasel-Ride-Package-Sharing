...
function buildBenchmarkPrice(route: CityRoute, region: RegionConfig) {
  ...
  return roundMoney(
    Math.max(5.5, fuelCostJod * 1.95 + route.distanceKm * 0.05 + tollAllowanceJod + 2.4),
  );
}
...
function buildModeledRow(route: CityRoute, region: RegionConfig): CorridorMarketRow {
  ...
  const ownershipScore = clamp(
    46 + tierBoost + launchBoost + popularityBoost + packageBoost,
    38,
    86,
  );
  ...
  return {
    ...
    sourceLine:
      'Launch model from route distance, regional fuel costs, corridor tier, and market status.',
  };
}
...
function buildLiveRow(route: CityRoute, region: RegionConfig): CorridorMarketRow | null {
  ...
  return {
    ...
    sourceLine:
      'Live production signals from rides, bookings, packages, alerts, and growth events.',
  };
}
...
export function buildCorridorMarketSnapshot(limit = 10): CorridorMarketSnapshot {
  const rows = getAllRegions()
    ...
    .flatMap(region =>
      pickKeyRoutes(region).map(route =>
        region.iso === 'JO'
    ...
    .sort((left, right) => {
      ...
      return (
        right.ownershipScore + right.savingsPercent - (left.ownershipScore + left.savingsPercent)
      );
    })
    .slice(0, limit);

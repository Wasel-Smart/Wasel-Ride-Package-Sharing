...
export async function fetchWithRetry(
  ...
  // If the breaker is already OPEN, reset it and let the request attempt the
  // direct-Supabase fallback path instead of blocking with a raw error. This
  // keeps the trust center and profile flows working when the edge transport
  // is degraded (e.g. CORS failures in local dev).
  if (breaker.getState() === CircuitState.OPEN) {
    breaker.reset();
  }

-- Reassert canonical auth signup sync for live projects where a legacy
-- auth.users trigger still inserts incomplete rows into public.users.
...
begin
  ...
  set
    ...
    phone_number = case
      when public.users.phone_number is null
        or public.users.phone_number = ''
        or public.users.phone_number like 'pending-%'

...
export function AppPill({ ar }: { ar: boolean }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 30, padding: '0 12px', borderRadius: R.full, background: C.cyanDim, border: `1px solid ${C.border}`, color: C.textSub, fontSize: '0.72rem', fontWeight: 700, fontFamily: F, whiteSpace: 'nowrap' }}>
      ...
      {ar ? 'Wasel for simpler movement' : 'Wasel for simpler movement'}
    </div>
  );
}
...
export function CurrencySwitcher({ ar }: { ar: boolean }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 200, background: C.bg, backdropFilter: 'blur(24px)', border: `1px solid ${C.border}`, borderRadius: 14, boxShadow: '0 16px 48px rgba(0,0,0,0.7)', overflow: 'hidden', zIndex: 1100, animation: 'fade-in 0.12s ease' }}>
          <div style={{ padding: '8px 12px 4px', fontSize: '0.6rem', fontWeight: 700, color: C.textMuted, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: F }}>
            {ar ? 'Select currency' : 'Select currency'}
          </div>
          {popular.map((code) => (
...
export function OnlineToggle({ ar }: { ar: boolean }) {
  ...
  return (
    <button onClick={() => setOnline((o) => !o)} title={online ? 'Go offline' : 'Go online'} style={{ height: 34, padding: '0 12px', borderRadius: R.full, background: online ? C.greenDim : C.card, border: `1.5px solid ${online ? `${C.green}66` : C.border}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem', fontWeight: 700, color: online ? C.green : C.textMuted, fontFamily: F, transition: 'all 0.2s' }}>
      ...
      {online ? (ar ? 'Online' : 'Online') : (ar ? 'Offline' : 'Offline')}
    </button>
  );
}
...
export function NavDropdown({ group, onNavigate, align, ar, isAuthenticated }: { group: NavGroup; onNavigate: (path: string) => void; align?: 'left' | 'center' | 'right'; ar: boolean; isAuthenticated: boolean; }) {
  ...
  return (
    <div role="menu" style={{ position: 'absolute', top: 'calc(100% + 10px)', ...posStyle, background: 'rgba(17,19,22,0.98)', backdropFilter: 'blur(28px)', border: `1px solid ${C.border}`, borderRadius: 18, boxShadow: '0 24px 64px rgba(0,0,0,0.65)', padding: 12, minWidth: 380, display: 'grid', gridTemplateColumns: cols, gap: 8, zIndex: 1000, animation: 'fade-in 0.15s ease' }}>
      {items.map((item) => (
...
export function UserMenu({ user, onSignOut, ar }: { user: { name: string; email: string; trips: number; balance: number }; onSignOut: () => void; ar: boolean; }) {
  ...
  const menuItems = [
    { label: ar ? 'Activity' : 'Activity', emoji: '•', path: '/my-trips' },
    { label: ar ? 'Packages' : 'Packages', emoji: '•', path: '/packages' },
    { label: ar ? 'Profile' : 'Profile', emoji: '•', path: '/profile' },
    { label: ar ? 'Wasel Plus' : 'Wasel Plus', emoji: '•', path: '/plus' },
  ];
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} aria-haspopup="true" aria-expanded={open} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 12px 5px 5px', borderRadius: 9999, background: open ? C.cyanDim : C.card, border: `1px solid ${open ? C.borderHov : C.border}`, cursor: 'pointer', transition: 'all 0.15s', backdropFilter: 'blur(12px)' }}>
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: GRAD, boxShadow: '0 0 0 1.5px rgba(184,138,82,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.68rem', fontWeight: 800, color: C.bg, flexShrink: 0 }}>{initials}</div>
      ...
      {open && (
        <div role="menu" style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 256, background: 'rgba(17,19,22,0.98)', backdropFilter: 'blur(28px)', border: `1px solid ${C.border}`, borderRadius: 16, boxShadow: '0 24px 64px rgba(0,0,0,0.75)', overflow: 'hidden', animation: 'fade-in 0.15s ease', zIndex: 1000 }}>
          <div style={{ padding: '14px 16px', background: 'rgba(184,138,82,0.08)', borderBottom: `1px solid ${C.border}` }}>
            ...
            <div style={{ display: 'flex', gap: 0, marginTop: 12, background: C.card, borderRadius: 10, overflow: 'hidden', border: `1px solid ${C.borderFaint}` }}>
              <div style={{ flex: 1, padding: '8px 12px', borderRight: `1px solid ${C.borderFaint}` }}>
                <div style={{ fontSize: '1rem', fontWeight: 900, color: C.text, fontFamily: F }}>{user.trips}</div>
                <div style={{ fontSize: '0.58rem', color: C.textMuted, fontFamily: F, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{ar ? 'Trips' : 'Trips'}</div>
              </div>
              <div style={{ flex: 1, padding: '8px 12px' }}>
                <div style={{ fontSize: '0.88rem', fontWeight: 900, color: C.gold, fontFamily: F }}>{balanceDisplay}</div>
                <div style={{ fontSize: '0.58rem', color: C.textMuted, fontFamily: F, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{ar ? 'Wallet' : 'Wallet'}</div>
              </div>
            </div>
          </div>
          {menuItems.map((item) => (
          ...
          <button onClick={() => { onSignOut(); setOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 16px', background: 'transparent', border: 'none', textAlign: 'left', fontSize: '0.82rem', fontWeight: 600, color: C.error, fontFamily: F, cursor: 'pointer' }}>
            <span style={{ fontSize: '1rem', width: 20, flexShrink: 0 }}>×</span>
            {ar ? 'Sign out' : 'Sign out'}
          </button>
        </div>
      )}
    </div>
...
export function LangToggle() {
  ...
  return (
    <button onClick={() => setLanguage(ar ? 'en' : 'ar')} title={ar ? 'Switch to English' : 'Switch to Arabic'} style={{ height: 34, padding: '0 10px', borderRadius: R.md, background: C.card, border: `1px solid ${C.border}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.75rem', fontWeight: 700, color: C.textSub, fontFamily: F, transition: 'all 0.14s' }}>
      {ar ? 'EN' : 'AR'}
    </button>
  );
}
...
export function MobileDrawer({ open, onClose, onNavigate, user, onSignOut, ar }: { open: boolean; onClose: () => void; onNavigate: (p: string) => void; user: { name: string; email: string } | null; onSignOut: () => void; ar: boolean; }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div style={{ position: 'absolute', top: 0, right: 0, width: 300, height: '100%', background: C.bg, borderLeft: `1px solid ${C.border}`, boxShadow: '-20px 0 60px rgba(0,0,0,0.7)', overflowY: 'auto', display: 'flex', flexDirection: 'column' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '16px 20px', borderBottom: `1px solid ${C.borderFaint}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ background: C.card, border: `1px solid ${C.borderFaint}`, borderRadius: R.md, width: 32, height: 32, cursor: 'pointer', fontSize: '1.1rem', color: C.textSub, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
        </div>
        ...
        {user && (
          <div style={{ padding: '14px 20px', background: 'rgba(184,138,82,0.08)', borderBottom: `1px solid ${C.borderFaint}` }}>
          ...
          {PRODUCT_NAV_GROUPS.filter((group) => isVisibleNavGroup(group, isAuthenticated)).map((group) => (
            <div key={group.id} style={{ padding: '12px 20px', borderBottom: `1px solid ${C.borderFaint}` }}>
              ...
              {'direct' in group && group.direct ? (
                <button onClick={() => { onNavigate((group as { path: string }).path); onClose(); }} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 0', background: 'none', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left' }}>
          ...
          {user ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <button onClick={() => { onNavigate('/auth?tab=register'); onClose(); }} style={{ height: 42, borderRadius: R.md, background: GRAD, border: 'none', color: C.bg, fontWeight: 700, fontFamily: F, fontSize: '0.875rem', cursor: 'pointer' }}>
                {ar ? 'Get started free' : 'Get started free'}
              </button>
            </div>
          )}
        </div>

...
export function AppPill({ ar }: { ar: boolean }) {
  return <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 30, padding: '0 12px', borderRadius: R.full, background: 'rgba(0,200,232,0.08)', border: '1px solid rgba(0,200,232,0.18)', color: 'rgba(239,246,255,0.82)', fontSize: '0.72rem', fontWeight: 700, fontFamily: F, whiteSpace: 'nowrap' }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: C.green, boxShadow: `0 0 10px ${C.green}` }} />{ar ? 'واصل لحركة الأردن' : 'Wasel for Jordan movement'}</div>;
}
...
export function OnlineToggle({ ar }: { ar: boolean }) {
  ...
  return <button onClick={() => setOnline((o) => !o)} title={ar ? (online ? 'الانتقال إلى وضع غير متصل' : 'الانتقال إلى وضع متصل') : online ? 'Go offline' : 'Go online'} style={{ height: 34, padding: '0 12px', borderRadius: R.full, background: online ? 'rgba(0,200,117,0.15)' : 'rgba(255,255,255,0.05)', border: `1.5px solid ${online ? 'rgba(0,200,117,0.45)' : 'rgba(255,255,255,0.15)'}`, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.72rem', fontWeight: 700, color: online ? C.green : 'rgba(239,246,255,0.55)', fontFamily: F, transition: 'all 0.2s' }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: online ? C.green : 'rgba(255,255,255,0.3)', boxShadow: online ? `0 0 8px ${C.green}` : 'none', flexShrink: 0, transition: 'all 0.2s' }} />{online ? (ar ? 'متصل' : 'Online') : (ar ? 'غير متصل' : 'Offline')}</button>;
}
...
export function NavDropdown({ group, onNavigate, align, ar, isAuthenticated }: { group: NavGroup; onNavigate: (path: string) => void; align?: 'left' | 'center' | 'right'; ar: boolean; isAuthenticated: boolean; }) {
  ...
  return <div role="menu" style={{ position: 'absolute', top: 'calc(100% + 10px)', ...posStyle, background: 'rgba(4,12,24,0.98)', backdropFilter: 'blur(28px)', border: '1px solid rgba(0,200,232,0.16)', borderRadius: 18, boxShadow: '0 24px 64px rgba(0,0,0,0.65), 0 0 0 1px rgba(0,200,232,0.06)', padding: 12, minWidth: 380, display: 'grid', gridTemplateColumns: cols, gap: 8, zIndex: 1000, animation: 'fade-in 0.15s ease' }}>
    {items.map((item) => <button key={item.label} role="menuitem" tabIndex={0} onClick={() => onNavigate(item.path)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onNavigate(item.path); } }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, padding: '11px 13px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer', textAlign: ar ? 'right' : 'left', transition: 'all 0.14s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: item.color, boxShadow: `0 0 10px ${item.color}55`, flexShrink: 0 }} />{item.badge && <Badge label={item.badge} color={item.color} />}</div>
...
export function UserMenu({ user, onSignOut, ar }: { user: { name: string; email: string; trips: number; balance: number }; onSignOut: () => void; ar: boolean; }) {
  ...
  const menuItems = [
    { label: ar ? 'نشاطي' : 'Activity', emoji: '•', path: '/my-trips' },
    { label: ar ? 'الطرود' : 'Packages', emoji: '•', path: '/packages' },
    { label: ar ? 'الملف الشخصي' : 'Profile', emoji: '•', path: '/profile' },
    ...
    {open && <div role="menu" style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 256, background: 'rgba(4,12,24,0.98)', backdropFilter: 'blur(28px)', border: '1px solid rgba(0,200,232,0.16)', borderRadius: 16, boxShadow: '0 24px 64px rgba(0,0,0,0.75)', overflow: 'hidden', animation: 'fade-in 0.15s ease', zIndex: 1000 }}>
      <div style={{ padding: '14px 16px', background: 'rgba(0,200,232,0.05)', borderBottom: '1px solid rgba(0,200,232,0.12)' }}>
        ...
        <div style={{ display: 'flex', gap: 0, marginTop: 12, background: 'rgba(255,255,255,0.04)', borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(0,200,232,0.10)' }}>
          <div style={{ flex: 1, padding: '8px 12px', borderRight: '1px solid rgba(0,200,232,0.10)' }}><div style={{ fontSize: '1rem', fontWeight: 900, color: '#EFF6FF', fontFamily: F }}>{user.trips}</div><div style={{ fontSize: '0.58rem', color: 'rgba(148,163,184,0.6)', fontFamily: F, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{ar ? 'الرحلات' : 'Trips'}</div></div>
      ...
      <button onClick={() => { onSignOut(); setOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 16px', background: 'transparent', border: 'none', textAlign: 'left', fontSize: '0.82rem', fontWeight: 600, color: '#FF4455', fontFamily: F, cursor: 'pointer' }}><span style={{ fontSize: '1rem', width: 20, flexShrink: 0 }}>×</span>{ar ? 'تسجيل الخروج' : 'Sign out'}</button>
    </div>}
  </div>;
}
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {PRODUCT_NAV_GROUPS.filter((group) => isVisibleNavGroup(group, isAuthenticated)).map((group) => (
          <div key={group.id} style={{ padding: '12px 20px', borderBottom: '1px solid rgba(0,200,232,0.06)' }}>
            ...
            {'direct' in group && group.direct ? <button onClick={() => { onNavigate((group as any).path); onClose(); }} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 0', background: 'none', border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left' }}><span aria-hidden="true" style={{ width: 8, height: 8, marginTop: 8, borderRadius: '50%', background: (group as any).color, boxShadow: `0 0 12px ${(group as any).color}55`, flexShrink: 0 }} /><span style={{ display: 'grid', gap: 4, flex: 1 }}><span style={{ fontSize: '0.875rem', fontWeight: 700, color: '#fff', fontFamily: F }}>{ar ? group.labelAr : group.label}</span><span style={{ fontSize: '0.72rem', color: 'rgba(148,163,184,0.65)', fontFamily: F, lineHeight: 1.5 }}>{ar ? (group as any).descAr : (group as any).desc}</span></span>{(group as any).badge && <Badge label={(group as any).badge} />}</button> : getVisibleNavItems(group, isAuthenticated).map((item) => <button key={item.label} onClick={() => { onNavigate(item.path); onClose(); }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', background: 'none', border: 'none', cursor: 'pointer', width: '100%' }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: item.color, boxShadow: `0 0 10px ${item.color}55`, flexShrink: 0 }} /><span style={{ fontSize: '0.84rem', fontWeight: 500, color: 'rgba(255,255,255,0.75)', fontFamily: F }}>{ar ? item.labelAr : item.label}</span>{item.badge && <Badge label={item.badge} color={item.color} />}</button>)}
          </div>
        ))}
      </div>

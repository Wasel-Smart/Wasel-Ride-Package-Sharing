...
export function usePushNotifications() {
  ...
  const [permission, setPermission] = useState<NotifPermission>(
    (NotificationApi?.permission ?? 'default') as NotifPermission,
  );
  ...
  const notify = useCallback(
    (options: NotifyOptions): Notification | null => {
      ...
      const createNotification = () => {
        const Ctor = NotificationApi as typeof Notification;
        const _body = typeof options.body === 'string' ? sanitizeHtml(options.body) : options.body;
        const init = {
          body: _body,
        ...
        try {
          return new Ctor(sanitizeHtml(options.title), init) as Notification;
        } catch {
          const callableCtor = Ctor as unknown as (
            title: string,
            options?: NotificationOptions,
          ) => Notification;
          return callableCtor(sanitizeHtml(options.title), init);
        }
      };

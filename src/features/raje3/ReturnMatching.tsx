      ...
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
            ...
            <div>
              ...
              >
                {isRTL ? 'رجّع — إرجاع ذكي' : 'Raje3 Smart Returns'}
              </h1>
              <p style={{ fontSize: '0.72rem', color: D.muted, margin: '3px 0 0' }}>
                {isRTL
                  ? 'مطابق مع الرحلات الحية التي تقبل الطرود والمتجهة إلى عمّان'
                  : 'Matched against live package-ready rides heading into Amman'}
              </p>
            </div>
          </div>
        </div>
          ...
          <AnimatePresence mode="wait">
            {step === 0 && (
              ...
              >
                ...
                >
                  {isRTL ? 'اختر المتجر' : 'Select your retailer'}
                </h2>
                <div
                  style={{
                    display: 'grid',
                ...
                >
                  {RETAILERS.map(retailerItem => (
                    <motion.div
                      ...
                      style={{
                        background:
                          retailer === retailerItem.id ? `${retailerItem.color}0E` : D.card2,
                    ...
                    >
                      ...
                      <div style={{ fontSize: '0.78rem', fontWeight: 700, color: D.text }}>
                        {isRTL ? retailerItem.nameAr : retailerItem.name}
                      </div>
                    </motion.div>
                  ))}
                </div>
            ...
            {step === 1 && (
              ...
              >
                ...
                >
                  {isRTL ? 'تفاصيل المنتج المُرجَع' : 'Return item details'}
                </h2>
                <div
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      placeholder={
                        isRTL ? 'مثال: حذاء رياضي أبيض، مقاس 42' : 'e.g. White sneakers, size 42'
                      }
                      style={{
                        width: '100%',
                        background: D.card2,
                      ...
                      {(
                        ...
                        <button
                          ...
                          style={{
                            ...
                            padding: '10px 0',
                            borderRadius: 10,
                            border: `1px solid ${size === key ? D.gold + '50' : D.border}`,
                            background: size === key ? `${D.gold}10` : D.card2,
                            color: size === key ? D.gold : D.sub,
                            fontSize: '0.78rem',
                            fontWeight: size === key ? 700 : 400,
                            fontFamily: D.F,
                            cursor: 'pointer',
                          }}
                        >
                      ...
                      {RETURN_REASONS.map(reasonItem => (
                        <button
                          ...
                          style={{
                            padding: '10px 14px',
                            borderRadius: 10,
                            border: `1px solid ${reason === reasonItem.id ? D.gold + '50' : D.border}`,
                            background: reason === reasonItem.id ? `${D.gold}10` : D.card2,
                            color: reason === reasonItem.id ? D.gold : D.sub,
                            fontSize: '0.82rem',
                            fontWeight: reason === reasonItem.id ? 700 : 400,
                            fontFamily: D.F,
                            cursor: 'pointer',
                            textAlign: 'left',
                            display: 'flex',
                  ...
                  >
                    {searching ? (
                      <>
                        ...
                        {isRTL ? 'جارٍ البحث…' : 'Finding matches…'}
                      </>
                    ) : (
                      <>
                        <Search size={15} />
                        {isRTL ? 'ابحث عن رحلة' : 'Find matching rides'}
                      </>
                    )}
                  </motion.button>
                </div>
            ...
            {step === 2 && (
              ...
              >
                <h2
                  style={{ fontSize: '1.05rem', fontWeight: 800, margin: '0 0 6px', color: D.text }}
                >
                  {isRTL ? 'رحلات مطابقة' : 'Matching rides'}
                </h2>
                <p style={{ fontSize: '0.76rem', color: D.muted, margin: '0 0 18px' }}>
                  {matches.length > 0
                    ? isRTL
                      ? `${matches.length} رحلة تدعم الطرود ويمكن استخدامها للإرجاع إلى عمّان`
                      : `${matches.length} package-ready rides can be used for this return into Amman`
                    : isRTL
                      ? 'لا توجد رحلات حية مطابقة حالياً، لكن سننشئ طلب إرجاع ونبقيه قيد البحث.'
                      : 'No live rides match right now, but we can still create the return request and keep it searching.'}
                </p>
                {matches.length > 0 ? (
                  <div
                  ...
                  >
                    {matches.map(match => (
                      <button
                        ...
                        style={{
                          background:
                            selectedMatch === match.id ? `rgba(240,168,48,0.07)` : D.card2,
                          border: `1px solid ${selectedMatch === match.id ? D.gold + '50' : D.border}`,
                          borderRadius: 14,
                          padding: '16px 18px',
                          cursor: 'pointer',
                          textAlign: 'left',
                  ...
                  >
                    ...
                    <span>
                      {isRTL
                        ? 'سيُنشأ الطلب بحالة بحث حتى تظهر رحلة مناسبة.'
                        : 'The return will be created in searching mode until a matching ride appears.'}
                    </span>
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8 }}>
            ...
            {step === 3 && createdReturn && (
              ...
              >
                <div style={{ textAlign: 'center', padding: '16px 0 24px' }}>
                  ...
                  >
                    {isRTL ? 'تم إنشاء طلب الإرجاع!' : 'Return Request Created!'}
                  </h2>
                  <p
                    style={{
                      fontSize: '0.8rem',
                  ...
                  >
                    {createdReturn.matchedRideId
                      ? isRTL
                        ? `تم ربط الطلب بمسار حي مع ${createdReturn.matchedDriver ?? 'سائق واصل'}.`
                        : `The request is already matched to a live route with ${createdReturn.matchedDriver ?? 'a Wasel captain'}.`
                      : isRTL
                        ? 'تم إنشاء الطلب بحالة بحث وسيظهر في تتبع الطرود حتى تتم المطابقة.'
                        : 'The request was created in searching mode and will stay visible in package tracking until matched.'}
                  </p>
                  <div
                    style={{
                      background: D.card2,
                  ...
                  >
                    ...
                    <div style={{ fontSize: '0.62rem', color: D.muted, marginTop: 4 }}>
                      {isRTL
                        ? 'استخدم هذا المعرّف للمتابعة في صفحة الطرود'
                        : 'Use this ID to continue tracking from the packages page'}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 10 }}>
                    <motion.button
                      whileHover={{ scale: 1.03 }}
                      onClick={() => nav('/app/raje3')}
                      style={{
                        flex: 1,
                        height: 44,
                        borderRadius: 12,
        ...
        >
          <div style={{ fontSize: '0.82rem', fontWeight: 700, color: D.gold, marginBottom: 16 }}>
            {isRTL ? 'كيف يعمل رجّع؟' : 'How Raje3 works'}
          </div>
          <div
            style={{
              display: 'grid',
          ...
          >
            {[
              ...
              {
                icon: '🤝',
                title: isRTL ? 'نربط الطلب بالشبكة' : 'We connect the request to the network',
              },
              {
                icon: '✅',
                title: isRTL ? 'نتابع بنفس معرّف التتبع' : 'One tracking ID follows the return',
              },
            ].map(item => (
              <div key={item.title} style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.72rem', color: D.sub, lineHeight: 1.5 }}>
                  {item.title}
                </div>
              </div>
            ))}

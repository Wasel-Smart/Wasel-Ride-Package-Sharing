...
const D = {
  ...
  border:'rgba(255,138,0,0.12)', gold:'#FF8A00', cyan:'#087DFF', green:'#39C72C',
              ...
              <motion.div key="s2" initial={{ opacity:0, x:16 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-16 }}>
                ...
                {matches.length > 0 ? (
                  <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:18 }}>
                    {matches.map(match => (
                      <button key={match.id} onClick={() => setSelectedMatch(match.id)} style={{ background: selectedMatch===match.id ? `rgba(255,138,0,0.07)` : D.card2, border:`1px solid ${selectedMatch===match.id ? D.gold+'50' : D.border}`, borderRadius:14, padding:'16px 18px', cursor:'pointer', textAlign:'left' }}>
            ...
            {step === 3 && createdReturn && (
              <motion.div key="done" initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}>
                <div style={{ textAlign:'center', padding:'16px 0 24px' }}>
                  <div style={{ width:70, height:70, borderRadius:'50%', background:'rgba(57,199,44,0.12)', border:`2px solid ${D.green}`, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px' }}>
                  ...
                  <div style={{ background:D.card2, border:`1px solid ${D.gold}30`, borderRadius:16, padding:'24px', marginBottom:20, display:'inline-block' }}>
                    <div style={{ width:120, height:120, background:'linear-gradient(135deg,rgba(255,138,0,0.15),rgba(8,125,255,0.10))', border:`2px solid ${D.gold}40`, borderRadius:12, display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 10px' }}>

    ...
    <Protected>
      <PageShell>
        ...
        <div className="sp-4col" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 14, marginBottom: 18 }}>
          {[
            { label: 'Verified accounts', value: 'Required', detail: 'Needed for trusted actions', color: DS.green },
            { label: 'Emergency help', value: 'Live', detail: 'SOS and trip support', color: '#EF4444' },
            { label: 'Coverage', value: 'Included', detail: 'Trip records included', color: DS.gold },
            { label: 'Comfort tools', value: 'Active', detail: 'Clear preferences', color: DS.cyan },
          ].map((item) => (
        ...
        <div className="sp-2col" style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 14, marginBottom: 18 }}>
          <div style={{ background: DS.card, borderRadius: r(20), padding: '22px', border: `1px solid ${DS.border}` }}>
            <div style={{ color: '#fff', fontWeight: 900, marginBottom: 10 }}>What riders see first</div>

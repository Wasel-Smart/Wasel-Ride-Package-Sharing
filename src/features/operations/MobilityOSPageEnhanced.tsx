...
function accentColor(accent: Accent) {
  return accent === 'green' ? DesignSystem.colors.green.base : accent === 'gold' ? DesignSystem.colors.gold.base : accent === 'purple' ? DesignSystem.colors.purple.base : DesignSystem.colors.cyan.base;
}
...
export default function MobilityOSPageEnhanced() {
  ...
  const stats = [
    { label: 'Dispatch IQ', value: `${network.dispatchIQ}`, detail: 'Weighted intelligence score for trust, speed, and balance.', accent: DesignSystem.colors.cyan.base },
    { label: 'Throughput', value: `${network.throughput} km`, detail: 'Predicted movement capacity across live corridors.', accent: DesignSystem.colors.green.base },
    { label: 'Balance Index', value: `${network.balance}%`, detail: 'Lower route variance means a calmer system feel.', accent: DesignSystem.colors.gold.base },
    { label: 'Carbon Gain', value: `${network.carbonGain} kg`, detail: 'Shared ride + parcel efficiency made visible.', accent: DesignSystem.colors.purple.base },
  ];
    ...
    <PageShell>
      ...
      <section style={{ display: 'grid', gap: 18, gridTemplateColumns: 'minmax(0, 1.5fr) minmax(320px, 0.9fr)' }}>
            ...
            <svg viewBox="0 0 100 110" style={{ width: '100%', height: '100%', display: 'block' }}>
                  ...
                  <g key={route.id}>
                    ...
                    {(tab === 'math' || tab === 'recovery') && (
                      <g>
                        ...
                        <text x={(from.x + to.x) / 2} y={(from.y + to.y) / 2 - 7.1} textAnchor="middle" fill={DesignSystem.colors.text.primary} fontSize="1.7">{route.score}</text>
                      </g>
                    )}
                  </g>
                );
              ...
              {NODES.map((node) => (
                <g key={node.id}>
                  ...
                  <text x={node.x} y={node.y - 8.4} textAnchor="middle" fill={DesignSystem.colors.text.primary} fontSize="2.5">{node.name}</text>
                  <text x={node.x} y={node.y + 10.2} textAnchor="middle" fill={DesignSystem.colors.text.muted} fontSize="1.45">{node.subtitle}</text>
                </g>
              ))}
              {UNITS
                  ...
                  return (
                    <g key={unit.id} transform={`translate(${x + nx} ${y + ny}) rotate(${rotation})`} opacity={clamp(0.52 + Math.sin(tick * 14 + index) * 0.22, 0.36, 0.98)}>
                      {unit.type === 'ride'
                        ...
                        : <rect x={-2} y={-2} width={4} height={4} rx={1.1} fill={DesignSystem.colors.gold.base} />}
                    </g>
                  );
                })}
            </svg>
        ...
        <div style={{ display: 'grid', gap: 14 }}>
          <DataPanel
            title="Corridor leaderboard"
            icon={<Gauge size={18} color={DesignSystem.colors.cyan.base} />}
          >
              ...
              {routeStats.slice(0, 3).map((route) => (
                <div key={route.id} style={{ padding: '12px 14px', borderRadius: 18, border: `1px solid ${accentColor(route.accent)}2a`, background: 'rgba(255,255,255,0.03)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'baseline' }}>
                    <div>
                      <div style={{ fontWeight: DesignSystem.typography.fontWeight.bold }}>{route.fromNode.name} → {route.toNode.name}</div>
                      <div style={{ marginTop: 4, color: DesignSystem.colors.text.muted, fontSize: DesignSystem.typography.fontSize.xs }}>{route.distance} km / ETA {route.etaLive} min</div>
                    </div>
                    <div style={{ fontSize: DesignSystem.typography.fontSize.xl, fontWeight: DesignSystem.typography.fontWeight.black, color: accentColor(route.accent) }}>{route.score}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <DataPanel
            ...
            icon={<Brain size={18} color={DesignSystem.colors.purple.base} />}
          >
            <div style={{ display: 'grid', gap: 10 }}>
              {[
      ...
      <section style={{ display: 'grid', gap: 14, gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
        {[
          { icon: TimerReset, title: 'Scenario tuning', value: TABS[tab], body: 'The UI changes by operational lens without hiding the network.', accent: DesignSystem.colors.cyan.base },
          { icon: Users, title: 'Agent synchrony', value: `${network.agentSync}%`, body: 'Ride and package layers are paced to feel orchestrated, not random.', accent: DesignSystem.colors.green.base },
          { icon: ShieldCheck, title: 'Recovery posture', value: `${network.resilience}%`, body: 'Risk is framed as control logic instead of generic warning noise.', accent: DesignSystem.colors.gold.base },

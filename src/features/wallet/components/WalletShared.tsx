...
const PAYMENT_TX_ICON: TxIconConfig = {
  icon: CreditCard,
  color: WaselColors.error,
  bg: 'bg-red-500/10',
};

export const TX_ICONS: Record<string, TxIconConfig> = {
  topup: { icon: Plus, color: WaselColors.success, bg: 'bg-green-500/10' },
  payment: PAYMENT_TX_ICON,
...
export function TransactionRow({
  ...
  const statusKey = String(transaction.status ?? 'completed').toLowerCase();
  const statusMeta = TX_STATUS_META[statusKey] ?? COMPLETED_STATUS_META;
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          ...
          {reference ? (
            <span className="text-[10px] text-muted-foreground/80">
              {translate('walletShared.ref')}
              {String(reference).slice(-8)}
            </span>
          ) : null}
        </div>
      </div>

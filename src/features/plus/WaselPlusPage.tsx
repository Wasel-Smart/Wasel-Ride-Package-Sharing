  ...
  return (
    <Protected>
      <PageShell>
        ...
        <CoreExperienceBanner
          title="Save more on repeat routes."
        ...
        >
          {[
            {
              label: 'Membership',
              value: membership.plusActive ? 'Active' : 'Inactive',
              detail: membership.plusActive ? 'Benefits are on' : 'Activate to unlock rewards',
              color: DS.gold,
            },
            {
              label: 'Credits',
              value: String(membership.movementCredits),
              detail: 'From rides and packages',
              color: DS.green,
            },
            {
              label: 'Streak',
              value: `${membership.streakDays} days`,
          ...
          >
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {[
                membership.plusActive
                  ...
                  : 'Activate Wasel Plus to unlock savings.',
                membership.commuterPassRoute
                  ? `Pass: ${membership.commuterPassRoute.label} for about ${membership.commuterPassRoute.subscriptionPriceJod} JOD.`
                  : 'Choose a commuter route.',
            ...
            <button
              ...
              style={{
                ...
                fontFamily: DS.F,
                cursor: 'pointer',
                boxShadow: `0 8px 24px ${DS.gold}35`,
              }}
            >
              {membership.plusActive ? 'Refresh plan' : 'Activate Wasel Plus'}
            </button>
          </div>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {habitPrograms.map(program => (
                ...
                >
                  ...
                  <div
                    style={{ color: DS.sub, fontSize: '0.75rem', lineHeight: 1.55, marginTop: 4 }}
                  >
                    {program.summary}
                  </div>
                  <div
                    style={{ color: DS.gold, fontSize: '0.72rem', fontWeight: 700, marginTop: 6 }}
                  >
                    {program.outcome}
                  </div>
                </div>
        ...
        >
          ...
          >
            {featuredCorridors.map(corridor => {
              ...
              const liveSignal =
                routeIntelligence.allSignals.find(signal => signal.id === corridor.id) ?? null;
              return (
                <div
                  key={corridor.id}
                  style={{
                ...
                >
                  ...
                  <div
                    style={{ color: DS.sub, fontSize: '0.75rem', lineHeight: 1.55, marginTop: 8 }}
                  >
                  ...
                  >
                    {[
                      {
                        label: 'Shared seat',
                        value: `${liveSignal?.priceQuote.finalPriceJod ?? corridor.sharedPriceJod} JOD`,
                      },
                      { label: 'Monthly pass', value: `${corridor.subscriptionPriceJod} JOD` },
                      {
                        label: 'Savings',
                        value: liveSignal
                          ? `${liveSignal.priceQuote.discountJod} JOD`
                          : `${corridor.savingsPercent}%`,
                      },
                    ].map(item => (
                      <div
                        key={item.label}
              ...
              {recurringSuggestions.map(suggestion => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      style={{ color: DS.sub, fontSize: '0.75rem', lineHeight: 1.55, marginTop: 6 }}
                    >
                      {suggestion.reason}
                    </div>
                    <div
                    ...
                    >
                      {suggestion.priceQuote.finalPriceJod} JOD now |{' '}
                      {suggestion.recommendedFrequency} at {suggestion.recommendedTime}
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                      ...
                      <button
                        onClick={() =>
                          nav(
                            `/app/find-ride?from=${encodeURIComponent(suggestion.from)}&to=${encodeURIComponent(suggestion.to)}&search=1`,
                          )
                        }
                        style={{
                          height: 38,

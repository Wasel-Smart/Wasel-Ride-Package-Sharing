    ...
    <Protected>
      <PageShell>
        ...
        <div className="sp-4col" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 14, marginBottom: 18 }}>
          {[
            { label: 'Membership', value: membership.plusActive ? 'Active' : 'Inactive', detail: membership.plusActive ? 'Benefits are on' : 'Activate to unlock rewards', color: DS.gold },
            { label: 'Credits', value: String(membership.movementCredits), detail: 'From rides and packages', color: DS.green },
            { label: 'Streak', value: `${membership.streakDays} days`, detail: 'Daily activity', color: DS.cyan },
            { label: 'Tier', value: tierLabel(membership.loyaltyTier), detail: dailySignal ? `${dailySignal.priceQuote.finalPriceJod} JOD daily route` : membership.commuterPassRoute?.label ?? 'No pass selected', color: DS.blue },
          ].map((item) => (

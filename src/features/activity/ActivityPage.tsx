...
function groupByDate(items: TimelineItem[]): { label: string; items: TimelineItem[] }[] {
  ...
  const groups: Record<string, TimelineItem[]> = { today: [], week: [], earlier: [] };
...
export function ActivityPage() {
  ...
  const items: TimelineItem[] = useMemo(() => {
    ...
    rides.forEach((b: RideBookingRecord) => {
      ...
      mapped.push({
        ...
        status: b.status,
        statusColor:
          b.status === 'completed' ? C.green
          : b.status === 'cancelled' || b.status === 'rejected' ? C.error
          : b.status === 'confirmed' ? C.cyan
          : C.gold,
        Icon: Car,
    ...
    packages.forEach((p: PackageRequest) => {
      mapped.push({
        ...
        status: p.status,
        statusColor: p.status === 'delivered' ? C.green : p.status === 'in_transit' ? C.cyan : C.gold,
        Icon: Package,
        path: '/app/packages',
      });
    });
    ...
    scheduledItems.forEach(s => {
      ...
      const typeLabel = s.item_type === 'ride'
        ? (ar ? 'رحلة مجدولة' : 'Scheduled ride')
        : s.item_type === 'package_delivery'
          ? (ar ? 'توصيل مجدول' : 'Scheduled delivery')
          : (ar ? 'إرجاع مجدول' : 'Scheduled return');
      mapped.push({
        id: s.id,
        kind: 'scheduled',
        title: typeLabel,
  ...
  const filterDefs: { key: typeof filter; label: string; labelAr: string }[] = [
    ...
    { key: 'ride',      label: `Rides (${countByKind['ride'] ?? 0})`,      labelAr: `رحلات (${countByKind['ride'] ?? 0})` },
    { key: 'package',   label: `Delivery (${countByKind['package'] ?? 0})`,labelAr: `توصيل (${countByKind['package'] ?? 0})` },
    { key: 'bus',       label: `Bus (${countByKind['bus'] ?? 0})`,         labelAr: `باص (${countByKind['bus'] ?? 0})` },
    { key: 'scheduled', label: `Scheduled (${countByKind['scheduled'] ?? 0})`, labelAr: `مجدول (${countByKind['scheduled'] ?? 0})` },
  ];
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            {filter !== 'all' && (() => {
              ...
              const KindIcon = { ride: Car, package: Package, bus: Bus, scheduled: Calendar }[filter as ItemKind] ?? Package;
              return (
                <>
                  <KindIcon size={32} color={C.textDim} />
                  <div style={{ marginTop: 12, color: C.textMuted, fontFamily: F, fontSize: TYPE.size.base }}>
                    {ar ? 'لا توجد عناصر بعد.' : 'Nothing here yet.'}
                  </div>
                  <button
            ...
            {filter === 'all' && (
              <>
                ...
                <div style={{ marginTop: 12, color: C.textMuted, fontFamily: F, fontSize: TYPE.size.base }}>
                  {ar ? 'لا توجد عناصر بعد.' : 'No activity yet.'}
                </div>
              </>
            )}
          </div>
                  ...
                  return (
                    ...
                    >
                      <div style={{
                        ...
                        color: item.statusColor, flexShrink: 0,
                      }}>
                        <item.Icon size={20} />
                      </div>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                          <span style={{
                            fontWeight: TYPE.weight.bold, color: C.text,
                        ...
                        <div style={{ color: C.textDim, fontSize: TYPE.size.xs, marginTop: 4, display: 'flex', gap: 10 }}>

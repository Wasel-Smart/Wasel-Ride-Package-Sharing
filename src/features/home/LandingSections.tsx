...
export function LandingHeroSection({ ar, emailAuthPath, signupAuthPath, findRidePath, mobilityOsPath, myTripsPath, supportLine, businessAddress, heroBullets, primaryActions, authError, oauthLoadingProvider, showQuickAuth = false, onGoogleAuth, onFacebookAuth, onNavigate }: LandingHeroSectionProps) {
  ...
  const highlights = [
    { label: ar ? 'Live confidence' : 'Live confidence', value: ar ? 'Route strength, timing, and network pressure in one read.' : 'Route strength, timing, and network pressure in one read.' },
    { label: ar ? 'One action layer' : 'One action layer', value: ar ? 'Find, create, or send without changing mental context.' : 'Find, create, or send without changing mental context.' },
  ] as const;
  return (
    <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ display: 'grid', gap: 16, height: '100%' }}>
          ...
          <div style={{ display: 'grid', gap: 18, alignContent: 'start' }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              <h1 style={{ margin: 0, maxWidth: 720, fontFamily: LANDING_DISPLAY, fontSize: 'clamp(2.7rem, 5vw, 5.15rem)', lineHeight: 0.94, letterSpacing: '-0.06em', fontWeight: 700 }}>
                <span style={{ display: 'block', color: LANDING_COLORS.text }}>{copy(ar ? 'A calmer way to read movement.' : 'A calmer way to read movement.')}</span>
                <span style={{ display: 'block', marginTop: 10, background: GRAD_SIGNAL, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{copy(ar ? 'Live routes, clear timing, one confident next step.' : 'Live routes, clear timing, one confident next step.')}</span>
              </h1>
              <p style={{ margin: 0, maxWidth: 620, color: LANDING_COLORS.muted, fontSize: '1rem', lineHeight: 1.74 }}>{copy(ar ? 'Wasel should feel precise and light. The first screen now tells one live mobility story instead of stacking noisy detail, so riders, drivers, and delivery users understand the network immediately.' : 'Wasel should feel precise and light. The first screen now tells one live mobility story instead of stacking noisy detail, so riders, drivers, and delivery users understand the network immediately.')}</p>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              <button type="button" onClick={() => onNavigate(mobilityOsPath || findRidePath)} style={{ minHeight: 54, padding: '0 22px', borderRadius: 18, border: '1px solid rgba(255,255,255,0.08)', background: GRAD_SIGNAL, color: '#041521', fontWeight: 900, fontSize: '0.95rem', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, boxShadow: SH.cyanL }}>{copy(ar ? 'Open Mobility OS live map' : 'Open Mobility OS live map')}<ArrowRight size={16} /></button>
              <button type="button" onClick={() => onNavigate(findRidePath)} style={{ minHeight: 54, padding: '0 20px', borderRadius: 18, border: `1px solid ${LANDING_COLORS.borderStrong}`, background: 'rgba(255,255,255,0.04)', color: LANDING_COLORS.text, fontWeight: 800, fontSize: '0.92rem', cursor: 'pointer' }}>{copy(ar ? 'Explore routes' : 'Explore routes')}</button>
          ...
          <div style={{ display: 'grid', gap: 14, alignContent: 'stretch' }}>
            <div style={{ position: 'relative', minHeight: 320, padding: '20px 12px', borderRadius: 34, background: 'radial-gradient(circle at 50% 38%, rgba(22,199,242,0.2), rgba(4,18,30,0) 28%), radial-gradient(circle at 56% 62%, rgba(199,255,26,0.18), rgba(4,18,30,0) 22%), linear-gradient(160deg, rgba(6,23,38,0.42) 0%, rgba(10,34,55,0.18) 52%, rgba(9,28,47,0.12) 100%)', boxShadow: '0 34px 80px rgba(1,10,18,0.24)', overflow: 'hidden', display: 'grid', placeItems: 'center' }}>
              ...
              <div style={{ position: 'relative', display: 'grid', gap: 14, justifyItems: 'center', textAlign: 'center' }}>
                <span style={{ color: LANDING_COLORS.cyan, fontSize: '0.74rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.14em', opacity: 0.96 }}>{copy(ar ? 'Wasel signal' : 'Wasel signal')}</span>
                ...
                <span style={{ maxWidth: 320, color: LANDING_COLORS.muted, fontSize: '0.88rem', lineHeight: 1.6 }}>{copy(ar ? 'A living identity that matches the same live mobility system shown in the map.' : 'A living identity that matches the same live mobility system shown in the map.')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {showQuickAuth && onGoogleAuth && onFacebookAuth ? (
        <div style={{ ...panel(28), padding: '18px', display: 'grid', gap: 14 }}>
          <div style={{ display: 'grid', gap: 6 }}>
            <span style={{ color: LANDING_COLORS.cyan, fontSize: '0.72rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{copy(ar ? 'Ø¯Ø®ÙˆÙ„ Ø³Ø±ÙŠØ¹' : 'Fast access')}</span>
            <div style={{ color: LANDING_COLORS.text, fontFamily: LANDING_DISPLAY, fontSize: '1.3rem', fontWeight: 700 }}>{copy(ar ? 'Ø§Ø¨Ø¯Ø£ Ø¨Ø®Ø·ÙˆØ© ÙˆØ§Ø­Ø¯Ø© ÙˆØ§Ø¶Ø­Ø©.' : 'Start in one clean step.')}</div>
            <p style={{ margin: 0, color: LANDING_COLORS.muted, fontSize: '0.9rem', lineHeight: 1.6 }}>{copy(ar ? 'Ø§Ø³ØªØ®Ø¯Ù… Google Ø£Ùˆ Facebook Ø£Ùˆ Ø§Ù„Ø¨Ø±ÙŠØ¯ Ø§Ù„Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠ Ù„Ù„ÙˆØµÙˆÙ„ Ø¥Ù„Ù‰ Ù†ÙØ³ ØªØ¬Ø±Ø¨Ø© Wasel.' : 'Use Google, Facebook, or email to enter the same Wasel flow.')}</p>
          </div>
          ...
          <div className="landing-auth-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
            {(['google', 'facebook'] as const).map((provider) => { const meta = AUTH_PROVIDER_META[provider]; const loading = oauthLoadingProvider === provider; return <button key={provider} type="button" onClick={provider === 'google' ? onGoogleAuth : onFacebookAuth} disabled={Boolean(oauthLoadingProvider)} style={{ minHeight: 54, borderRadius: 18, border: `1px solid ${meta.accent}40`, background: `${meta.accent}12`, color: LANDING_COLORS.text, padding: '0 16px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontSize: '0.9rem', fontWeight: 800, cursor: oauthLoadingProvider ? 'not-allowed' : 'pointer', opacity: oauthLoadingProvider && !loading ? 0.55 : 1 }}><AuthProviderBadge provider={provider} size={20} /><span>{loading ? (ar ? `Ø¬Ø§Ø± Ø§Ù„Ø§ØªØµØ§Ù„ Ø¹Ø¨Ø± ${meta.label}...` : `Connecting ${meta.label}...`) : (ar ? `ØªØ§Ø¨Ø¹ Ø¹Ø¨Ø± ${meta.label}` : `Continue with ${meta.label}`)}</span></button>; })}
      ...
      <div className="landing-footer-meta" style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', color: LANDING_COLORS.soft, fontSize: '0.82rem' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: LANDING_COLORS.text }}><span className="landing-live-dot" style={{ width: 8, height: 8, borderRadius: '50%', background: LANDING_COLORS.green, boxShadow: `0 0 12px ${LANDING_COLORS.green}` }} />{copy(ar ? 'ØªØ­Ø¯ÙŠØ« Ø­ÙŠ Ù„Ù„Ù…Ù…Ø±Ø§Øª' : 'Live corridor refresh')}</span>

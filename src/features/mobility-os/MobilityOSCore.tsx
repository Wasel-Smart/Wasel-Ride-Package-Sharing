...
const PASSENGER_COLOR = '#087DFF';
const PACKAGE_COLOR = '#FF8A00';
const PASSENGER_GLOW = 'rgba(8,125,255,0.45)';
const PACKAGE_GLOW = 'rgba(255,138,0,0.34)';
...
function hourPalette(hour: number) {
  ...
  return { top: '#020914', bottom: '#071423', glow: 'rgba(8,125,255,0.12)' };
}
...
export default function MobilityOSCore() {
  ...
  const drawScene = useCallback(() => {
    ...
    atmosphere.addColorStop(0, viewMode === 'pulse' ? 'rgba(167,124,255,0.18)' : 'rgba(8,125,255,0.18)');
    atmosphere.addColorStop(0.35, viewMode === 'pulse' ? 'rgba(167,124,255,0.08)' : 'rgba(8,125,255,0.06)');
    atmosphere.addColorStop(1, 'rgba(8,125,255,0)');
    ctx.fillStyle = atmosphere;
    ctx.fillRect(0, 0, width, height);
    const amberGlow = ctx.createRadialGradient(width * 0.78, height * 0.82, 0, width * 0.78, height * 0.82, width * 0.42);
    amberGlow.addColorStop(0, viewMode === 'satellite' ? 'rgba(102,244,198,0.08)' : 'rgba(255,138,0,0.12)');
    amberGlow.addColorStop(1, 'rgba(255,138,0,0)');
    ...
    energyRibbon.addColorStop(0.35, 'rgba(8,125,255,0.025)');
    energyRibbon.addColorStop(0.55, 'rgba(255,255,255,0.045)');
    energyRibbon.addColorStop(0.7, 'rgba(255,138,0,0.022)');
    ...
    horizonShelf.addColorStop(0, 'rgba(8,125,255,0)');
    horizonShelf.addColorStop(0.5, 'rgba(8,125,255,0.05)');
    ...
    ctx.strokeStyle = viewMode === 'pulse' ? 'rgba(167,124,255,0.04)' : 'rgba(8,125,255,0.026)';
    ...
    terrain.addColorStop(1, 'rgba(255,138,0,0.05)');
    ...
    reliefWash.addColorStop(0.4, 'rgba(8,125,255,0.03)');
    reliefWash.addColorStop(1, 'rgba(255,138,0,0.03)');
    ...
    scan.addColorStop(0.45, 'rgba(8,125,255,0.08)');
    scan.addColorStop(1, 'rgba(8,125,255,0)');
    ctx.fillStyle = scan;
    ctx.beginPath();
    ...
    routesRef.current.forEach((route, routeIndex) => {
      ...
      passengerGradient.addColorStop(0.5, viewMode === 'pulse' ? `rgba(167,124,255,${0.38 + route.passengerFlow / 3400})` : `rgba(8,125,255,${0.34 + route.passengerFlow / 3200})`);
      ...
      ctx.moveTo(from.x, from.y);
      ctx.quadraticCurveTo(curve.cx, curve.cy, to.x, to.y);
      ctx.strokeStyle = viewMode === 'satellite' ? `rgba(114,255,213,${0.16 + route.packageFlow / 1500})` : `rgba(255,138,0,${0.18 + route.packageFlow / 1400})`;
      ...
      if (routeIndex % 3 === 0) {
        ...
        ctx.strokeStyle = 'rgba(8,125,255,0.18)';
    ...
    [...routesRef.current]
      ...
      .forEach((route, rank) => {
        ...
        ctx.fill();
        ctx.shadowBlur = 0;
        ctx.strokeStyle = rank === 0 ? 'rgba(8,125,255,0.42)' : 'rgba(255,255,255,0.16)';
        ctx.lineWidth = 1;
        ctx.stroke();
    ...
    vehiclesRef.current.forEach((vehicle) => {
      ...
      if (vehicle.isLiveTelemetry) {
        ...
        ctx.strokeStyle = vehicle.freshness === 'fresh' ? 'rgba(92,255,149,0.9)' : 'rgba(255,138,0,0.82)';
        ctx.lineWidth = 1.4;
        ctx.stroke();
      }
    ...
    CITY_DATA.forEach((city) => {
      ...
      halo.addColorStop(0, selected ? 'rgba(255,255,255,0.34)' : city.isHub ? 'rgba(8,125,255,0.26)' : 'rgba(255,255,255,0.16)');
      ...
      ctx.fillStyle = selected ? 'rgba(255,255,255,0.98)' : city.isHub ? 'rgba(8,125,255,0.98)' : 'rgba(255,255,255,0.82)';
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(point.x, point.y - (selected ? 26 : city.isHub ? 22 : 18));
      ctx.lineTo(point.x, point.y - 6);
      ctx.strokeStyle = selected ? 'rgba(255,138,0,0.34)' : city.isHub ? 'rgba(8,125,255,0.28)' : 'rgba(255,255,255,0.12)';
      ctx.lineWidth = selected ? 2 : 1.3;
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(point.x, point.y, selected ? 4.6 : city.isHub ? 3.6 : 2.8, 0, Math.PI * 2);
      ctx.fillStyle = selected ? 'rgba(255,138,0,0.94)' : 'rgba(255,255,255,0.95)';
      ctx.fill();
      ctx.beginPath();
      ctx.arc(point.x, point.y, selected ? 14 : city.isHub ? 11 : 8, 0, Math.PI * 2);
      ctx.strokeStyle = selected ? 'rgba(255,138,0,0.82)' : city.isHub ? 'rgba(8,125,255,0.34)' : 'rgba(255,255,255,0.18)';
      ...
      ctx.shadowColor = selected ? 'rgba(255,138,0,0.18)' : 'rgba(8,125,255,0.08)';
      ctx.fill();
      ctx.shadowBlur = 0;
      ctx.strokeStyle = selected ? 'rgba(255,138,0,0.42)' : 'rgba(255,255,255,0.1)';
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      ctx.strokeStyle = selected ? 'rgba(255,138,0,0.32)' : 'rgba(255,255,255,0.12)';
      ctx.lineWidth = 1;
      ctx.stroke();
    ...
    ctx.strokeStyle = 'rgba(8,125,255,0.16)';
    ...
    [
      ...
      ctx.strokeStyle = 'rgba(8,125,255,0.34)';
      ctx.lineWidth = 2;
      ctx.stroke();
    });
  ...
  return (
    <div dir={dir} style={{ minHeight: '100vh', background: `${GRAD_AURORA}, radial-gradient(circle at 15% 12%, rgba(8,125,255,0.16), transparent 22%), radial-gradient(circle at 82% 18%, rgba(255,138,0,0.14), transparent 24%), radial-gradient(circle at 50% 100%, rgba(92,255,149,0.08), transparent 28%), ${C.bg}`, color: C.text, fontFamily: F, padding: '20px 14px 88px' }}>
      <div style={{ maxWidth: 1460, margin: '0 auto', display: 'grid', gap: 18 }}>
        <section style={glassPanelStyle({ padding: 28, borderRadius: 34 })}>
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(135deg, rgba(255,255,255,0.04), transparent 24%, transparent 72%, rgba(8,125,255,0.08))' }} />
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: viewMode === mode.id ? 'rgba(8,125,255,0.14)' : 'rgba(255,255,255,0.035)',
                      ...
                      fontSize: '0.72rem',
                      boxShadow: viewMode === mode.id ? '0 0 24px rgba(8,125,255,0.18)' : 'none',
                    }}
                  >
                    {mode.label}
                  </button>
        ...
        <section style={{ display: 'grid', gap: 18 }}>
          <div style={glassPanelStyle({ padding: 20, borderRadius: 30, background: 'linear-gradient(180deg, rgba(8,20,37,0.97), rgba(4,10,22,0.99))', alignSelf: 'start' })}>
            ...
            <div ref={wrapRef} style={{ ...glassPanelStyle({ padding: 0, borderRadius: 34, aspectRatio: `${HERO_MAP_ASPECT} / 1`, minHeight: 'clamp(500px, 54vw, 860px)', boxShadow: '0 60px 160px rgba(0,0,0,0.54), inset 0 1px 0 rgba(255,255,255,0.08)', transform: 'perspective(2200px) rotateX(5deg)', transformStyle: 'preserve-3d', transformOrigin: 'center top' }), background: 'linear-gradient(180deg, rgba(6,16,28,0.98), rgba(5,12,22,0.98))' }}>
              <div style={{ position: 'absolute', inset: -30, pointerEvents: 'none', background: 'radial-gradient(circle at 50% 8%, rgba(255,255,255,0.07), transparent 24%), radial-gradient(circle at 14% 30%, rgba(8,125,255,0.14), transparent 20%), radial-gradient(circle at 84% 26%, rgba(255,138,0,0.12), transparent 22%)', filter: 'blur(18px)', opacity: 0.9 }} />
              <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'linear-gradient(135deg, rgba(255,255,255,0.04), transparent 18%, transparent 82%, rgba(8,125,255,0.06))' }} />
              <div style={{ position: 'absolute', inset: 14, borderRadius: 22, border: '1px solid rgba(255,255,255,0.06)', pointerEvents: 'none' }} />
              <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(circle at top center, rgba(255,255,255,0.08), transparent 22%), radial-gradient(circle at bottom right, rgba(8,125,255,0.08), transparent 28%)' }} />
              ...
              <div style={{ position: 'absolute', left: 24, right: 24, bottom: -22, height: 38, borderRadius: 999, pointerEvents: 'none', background: 'linear-gradient(90deg, rgba(8,125,255,0), rgba(8,125,255,0.24), rgba(255,138,0,0.18), rgba(8,125,255,0.24), rgba(8,125,255,0))', filter: 'blur(16px)', opacity: 0.8 }} />
              ...
              <div style={{ position: 'absolute', left: '50%', bottom: 20, transform: 'translateX(-50%)', zIndex: 2, pointerEvents: 'none' }}>
                <div style={{ width: 180, height: 18, borderRadius: 999, background: 'linear-gradient(90deg, rgba(8,125,255,0), rgba(8,125,255,0.16), rgba(255,138,0,0.16), rgba(255,138,0,0))', filter: 'blur(10px)' }} />
              </div>
              ...
              <div style={{ position: 'absolute', right: 22, top: '50%', transform: 'translateY(-50%)', zIndex: 2, writingMode: 'vertical-rl', textOrientation: 'mixed', letterSpacing: '0.28em', textTransform: 'uppercase', fontSize: '0.64rem', color: 'rgba(8,125,255,0.34)', pointerEvents: 'none' }}>
                {copy.signalLayer}
              </div>
              <canvas
                ref={canvasRef}
          ...
          <div style={{ display: 'grid', gap: 14, gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' }}>
            <article style={glassPanelStyle({ padding: 18, borderRadius: 26, background: 'linear-gradient(180deg, rgba(8,24,38,0.96), rgba(4,10,22,0.98))' })}>
              ...
              <div style={{ marginTop: 14, display: 'grid', gap: 10 }}>
                {[
                  ...
                  <div key={row} style={{ padding: '10px 12px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.07)', background: index === 2 ? 'rgba(8,125,255,0.08)' : 'rgba(255,255,255,0.03)', color: index === 2 ? C.cyan : C.textSub, fontWeight: index === 2 ? 700 : 500 }}>
                    {row}
                  </div>
                ))}
              </div>
            ...
            <article style={glassPanelStyle({ padding: 18, borderRadius: 26, background: 'linear-gradient(180deg, rgba(9,19,35,0.96), rgba(4,10,22,0.98))' })}>
              ...
              <div style={{ marginTop: 14, display: 'grid', gap: 10 }}>{[analytics.dispatchAction, `${copy.topCorridor}: ${analytics.topCorridor}`, ar ? `توفر المقاعد: ${numberFormatter.format(analytics.seatAvailability)} / سعة الطرود: ${numberFormatter.format(analytics.packageCapacity)}` : `Seat availability: ${analytics.seatAvailability} / Package capacity: ${analytics.packageCapacity}`].map((item, index) => <div key={item} style={{ padding: '12px 14px', borderRadius: 16, background: index === 0 ? 'linear-gradient(135deg, rgba(255,138,0,0.12), rgba(255,255,255,0.03))' : 'rgba(255,255,255,0.03)', border: `1px solid ${index === 0 ? 'rgba(255,138,0,0.24)' : C.borderFaint}`, color: C.textSub, lineHeight: 1.6, boxShadow: index === 0 ? '0 10px 24px rgba(255,138,0,0.08)' : 'none' }}>{item}</div>)}</div>
          ...
          <article style={glassPanelStyle({ padding: 22, borderRadius: 32, background: 'linear-gradient(180deg, rgba(8,18,34,0.98), rgba(4,10,22,1))' })}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {visibleRoutes.map((route, index) => {
                ...
                const pressureTone = route.congestion > 0.75 ? 'rgba(255,120,92,0.16)' : route.packageFlow > route.passengerFlow * 0.45 ? 'rgba(255,138,0,0.14)' : 'rgba(8,125,255,0.12)';
                return (
                  <div key={route.id} style={{ position: 'relative', padding: '18px 18px 16px', borderRadius: 24, border: `1px solid ${index === 0 ? C.cyanGlow : C.borderFaint}`, background: `linear-gradient(180deg, ${pressureTone}, rgba(255,255,255,0.025))`, boxShadow: index === 0 ? '0 18px 40px rgba(8,125,255,0.12)' : '0 10px 30px rgba(0,0,0,0.16)' }}>
                    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: `radial-gradient(circle at top right, ${index === 0 ? 'rgba(8,125,255,0.16)' : 'rgba(255,255,255,0.05)'}, transparent 32%)` }} />
                    ...
                    <div style={{ position: 'relative', display: 'grid', gap: 14 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                        <div style={{ display: 'grid', gap: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                            <div style={{ width: 34, height: 34, borderRadius: 12, background: index === 0 ? 'rgba(8,125,255,0.16)' : 'rgba(255,255,255,0.06)', border: `1px solid ${index === 0 ? 'rgba(8,125,255,0.24)' : 'rgba(255,255,255,0.08)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, color: index === 0 ? C.cyan : C.textSub }}>
                        ...
                        <div style={{ display: 'grid', gap: 8, minWidth: 120 }}>
                          ...
                          <div style={{ textAlign: 'right', color: index === 0 ? C.cyan : C.text, fontWeight: 900, fontSize: '2rem', lineHeight: 1, textShadow: index === 0 ? '0 0 18px rgba(8,125,255,0.22)' : 'none' }}>
                            {routeScore}
                          </div>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
                            {[
                          { label: `${copy.passengerOccupancy} · ${copy.estimateTag}`, value: route.passengerFlow / Math.max(route.lanes * 1800, 1), color: PASSENGER_COLOR, tone: 'rgba(8,125,255,0.08)' },
                          { label: `${copy.packageUtilization} · ${copy.estimateTag}`, value: route.packageFlow / Math.max(route.lanes * 820, 1), color: PACKAGE_COLOR, tone: 'rgba(255,138,0,0.08)' },

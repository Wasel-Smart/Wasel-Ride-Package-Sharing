...
export function canPoolRides(
  ...
  if (
    routeA.corridorsUsed.length > 0 &&
    routeA.corridorsUsed.some(r => routeB.corridorsUsed.includes(r))
  ) {
    const shared = routeA.corridorsUsed.find(r => routeB.corridorsUsed.includes(r));
    return {
      canPool: true,
      detourKm: Math.abs(routeA.totalDistanceKm - routeB.totalDistanceKm),
      sharedCorridor: shared ?? null,
    };
  }
  ...
  for (const stopA of routeA.orderedStops) {
    for (const stopB of routeB.orderedStops) {
      if (stopA === stopB) {
        ...
        return {
          canPool: true,
          detourKm: Math.min(routeA.totalDistanceKm, routeB.totalDistanceKm) * 0.15,
          sharedCorridor: corridor?.id ?? null,
        };
      }
    }
  }
...
export function validateBooking(
  ...
  if (!driver.allowsPackages) {
    ...
    issues.push(
      `Cargo capacity exceeded: need ${totalCargoNeeded}kg, available ${driver.cargoCapacityKg}kg`,
    );
  }
  ...
  for (const pkg of packages) {
    const arrival =
      ...
      (JORDAN_DISTANCES[driver.destinationCity]?.[pkg.destinationCity] ?? 0) * 60_000;
...
export function runMobilityStep(input: {
  ...
  const corridorMetrics: CorridorMetrics[] = dispatch.routes.map(route => {
    ...
    return {
      ...
      seatPressure: corridorPressure(city.outgoingDemand, route.baseSeatCapacity),

      cargoPressure: corridorPressure(city.packageFlow, route.baseCargoCapacityKg),
...
export function generateCityGraph(): {
  ...
  const nodes = Object.keys(JORDAN_DISTANCES).filter((k): k is City => k in JORDAN_DISTANCES);
...
export function getDistance(origin: City, destination: City): number {
  return (
    JORDAN_DISTANCES[origin]?.[destination] ?? JORDAN_DISTANCES[destination]?.[origin] ?? Infinity
  );
}

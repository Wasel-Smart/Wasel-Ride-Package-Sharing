...
// Pricing and calculation constants
const SOLO_PRICE_MULTIPLIER = 1.75;
const DISTANCE_PRICE_FACTOR = 0.02;
const BASE_PRICE_ADDITION = 1.8;
const TOTAL_TRIP_COST_FACTOR = 0.88;
const DENSITY_DISCOUNT_HIGH_DEMAND = 0.92; // for demandScore >= 90
const DENSITY_DISCOUNT_MEDIUM_DEMAND = 0.96; // for demandScore >= 80
const DRIVER_BOOST_SHARED_PRICE_FACTOR = 0.18;
const DRIVER_BOOST_ATTACH_RATE_FACTOR = 0.04;
const SUBSCRIPTION_PRICE_MONTHLY_TRIPS = 18;
const SUBSCRIPTION_PRICE_SURGING_DISCOUNT = 0.92;
const SUBSCRIPTION_PRICE_DEFAULT_DISCOUNT = 0.97;
const MIN_SAVINGS_PERCENT = 20;

/**
 * Calculates various pricing components for a given JordanRoute based on demand score.
 * @param route The JordanRoute object containing base fare, distance, etc.
 * @param demandScore The predicted demand score for the corridor, influencing discounts.
 * @returns An object containing solo reference price, shared price, savings percentage, and the full price ladder.
 */
function buildRoutePrices(route: JordanRoute, demandScore: number) {
  const soloReferencePriceJod = roundMoney(route.baseFare * SOLO_PRICE_MULTIPLIER + route.distance * DISTANCE_PRICE_FACTOR + BASE_PRICE_ADDITION);
  const totalTripCost = Math.max(route.baseFare * 1.2, soloReferencePriceJod * TOTAL_TRIP_COST_FACTOR);
  const priceLadder = SmartPricingEngine.calculateSharedRidePricing(totalTripCost, 4);
  const densityDiscount = demandScore >= 90 ? DENSITY_DISCOUNT_HIGH_DEMAND : demandScore >= 80 ? DENSITY_DISCOUNT_MEDIUM_DEMAND : 1;
...
/**
 * Builds a comprehensive CorridorOpportunity object from a JordanRoute and optional origin/destination.
 * This function integrates route details, meta-information, and pricing calculations.
 * @param route The base JordanRoute object.
 * @param from Optional override for the origin city label.
 * @param to Optional override for the destination city label.
 * @returns A CorridorOpportunity object with all calculated and aggregated details.
 */
function buildOpportunity(route: JordanRoute, from?: string, to?: string): CorridorOpportunity {
  ...
  const driverBoostJod = roundMoney(
    prices.sharedPriceJod * meta.fillTargetSeats * DRIVER_BOOST_SHARED_PRICE_FACTOR + meta.attachRatePercent * DRIVER_BOOST_ATTACH_RATE_FACTOR,
  );
  const subscriptionPriceJod = roundMoney(
    prices.sharedPriceJod * SUBSCRIPTION_PRICE_MONTHLY_TRIPS * (meta.density === 'surging' ? SUBSCRIPTION_PRICE_SURGING_DISCOUNT : SUBSCRIPTION_PRICE_DEFAULT_DISCOUNT),
  );
...
/**
 * Builds a DriverRoutePlan, providing financial insights for a driver offering a route.
 * This includes recommended pricing, potential gross earnings, and empty seat costs.
 * @param from The origin city of the route.
 * @param to The destination city of the route.
 * @param seats The number of seats the driver plans to offer.
 * @returns A DriverRoutePlan object or null if the corridor cannot be found.
 */
export function buildDriverRoutePlan(from: string, to: string, seats: number) {
  ...
  const priceModel = SmartPricingEngine.calculateSharedRidePricing(
    ...
    Math.max(2, seats), // Ensure at least 2 seats for pricing model
  );

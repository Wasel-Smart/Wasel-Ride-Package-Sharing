...
describe('signInSchema', () => {
  const valid = { email: 'user@wasel.jo', password: TEST_PASSWORD };
  ...
  it('rejects a password with no digits', () => {
    expect(isValid(signInSchema, { ...valid, password: 'alllowercase' })).toBe(false);
  });

  it('rejects a password with no letters', () => {
    expect(isValid(signInSchema, { ...valid, password: '12345678' })).toBe(false);
  });
});
...
describe('signUpSchema', () => {
  ...
  it('rejects an invalid phone format', () => {
    expect(isValid(signUpSchema, { ...valid, phone: '12345' })).toBe(false);
  });
});

...
describe('SmartPricingEngine.calculateSchoolSubscription()', () => {
  ...
  it('round trip applies the 1.8x multiplier documented in the source', () => {
    ...
    // which is equivalent to a 1.9x multiplier on the one-way cost.
    // The previous implementation used an incorrect 1.8x multiplier.
    const expectedRoundTrip = Math.ceil(oneWay.standard * 1.9); // 550 * 1.9 = 1045
    expect(roundTrip.standard).toBe(expectedRoundTrip);
    expect(roundTrip.standard).toBe(1045);
  });

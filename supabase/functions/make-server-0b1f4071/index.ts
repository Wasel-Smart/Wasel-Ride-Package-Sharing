...
import { type DeliveryProcessorEnv } from './_shared/communication-runtime.ts';
import {
...
function noContent(status = 204) {
  return noContentResponse(corsHeaders, status);
}
...
const {
...
} = createCommunicationHandlers(
  {
    ...
    getAdminClient,
    getFunctionBaseUrl,
    hasCommunicationWorkerAccess,
    json,
  },
  {
    deliveryEnv,
    communicationsRuntimeSql: COMMUNICATIONS_RUNTIME_SQL,
    communicationsOperationsSql: COMMUNICATIONS_OPERATIONS_SQL,
  },
);
...
const {
  ...
  getAdminClient,
  getFunctionBaseUrl,
  hasAutomationWorkerAccess,
  json,
  processQueuedDeliveries,
}, {
  automationRuntimeSql: AUTOMATION_RUNTIME_SQL,
});
...
const routes: RouteDefinition[] = [
  ...
  { method: 'POST', path: '/payments/status', handler: handleGetPaymentStatus },
...
Deno.serve((request) =>
  dispatchRouteRequest({
    corsHeaders,
    request,
    basePathPattern: /^.*make-server-0b1f4071/,
    routes,
  }));

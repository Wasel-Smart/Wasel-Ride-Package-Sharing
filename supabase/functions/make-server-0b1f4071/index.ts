...
function mapMobilityCorridorRow(row: Record<string, unknown>): MobilityCorridorRow {
  return {
    id: String(row.id ?? ''),
    origin: String(row.origin ?? ''),
    destination: String(row.destination ?? ''),
    distance_km: Number(row.distance_km ?? 0),
    travel_time_min: Number(row.travel_time_min ?? 0),
    seats_total: Number(row.seats_total ?? 0),
    seats_booked: Number(row.seats_booked ?? 0),
    cargo_total_kg: Number(row.cargo_total_kg ?? 0),
    cargo_booked_kg: Number(row.cargo_booked_kg ?? 0),
    base_price_seat: Number(row.base_price_seat ?? 0),
    base_price_kg: Number(row.base_price_kg ?? 0),
    demand_index: Number(row.demand_index ?? 0),
    demand_history: parseNumericArray(row.demand_history),
    price_history: parseNumericArray(row.price_history),
    updated_at: String(row.updated_at ?? new Date().toISOString()),
  };
}

function createMobilityEventEnvelope(input: {
  traceId: string;
  producer: string;
  type: string;
...
async function ensureMobilityOsRuntime(): Promise<void> {
  await executeSqlStatements(MOBILITY_OS_RUNTIME_SQL);
  await executeSqlStatements(MOBILITY_OS_SEED_SQL);
}
...
async function handleApplyMobilityOsMigrations(request: Request) {
  if (!hasWorkerAccess(request)) {
    return json({ error: 'Missing worker secret' }, 401);
  }

  await ensureMobilityOsRuntime();

  return json({
    applied: [
      'mobility_os_runtime_contract',
      'mobility_os_seed_corridors',
    ],
  });
}

async function handleGetMobilitySnapshot(request: Request) {
  const auth = await authenticateRequest(request);
  if ('error' in auth) return auth.error;

  await ensureMobilityOsRuntime();

  const snapshot = await withDatabaseClient(async (client) => {
    const corridorResult = await client.queryObject<Record<string, unknown>>(
      'select * from public.mobility_corridors order by origin, destination',
    );
    const eventsResult = await client.queryObject<Record<string, unknown>>(
      `select id, event_type as type, trace_id, payload, occurred_at
       from public.mobility_event_outbox
       order by occurred_at desc
       limit 25`,
    );

    const corridors = corridorResult.rows.map(mapMobilityCorridorRow);
    const recentEvents = eventsResult.rows.map((row) => ({
      id: row.id,
      type: row.type,
      occurred_at: row.occurred_at,
      trace_id: row.trace_id,
      producer: (row.payload as Record<string, unknown> | null)?.producer ?? 'MobilityOS',
      payload: (row.payload as Record<string, unknown> | null)?.payload ?? row.payload ?? {},
    }));

    return buildMobilitySnapshot(corridors, recentEvents);
  });

  return json({
    source: 'mobility-os-server',
    ...snapshot,
  });
}

async function handleMobilityBookingCreate(request: Request) {
  const auth = await authenticateRequest(request);
  if ('error' in auth) return auth.error;
  ...
  const body = await request.json().catch(() => ({}));
  const corridorId = String(body?.corridor_id ?? '').trim();
  const type = String(body?.type ?? '').trim() as MobilityBookingType;
  const quantity = Number(body?.quantity ?? 0);
  const timestamp = String(body?.timestamp ?? new Date().toISOString());

  if (!corridorId) {
    return json({ error: 'corridor_id is required.' }, 400);
  }
  if (type !== 'seat' && type !== 'cargo') {
    return json({ error: 'type must be seat or cargo.' }, 400);
  }
  if (!Number.isFinite(quantity) || quantity <= 0) {
    return json({ error: 'quantity must be greater than zero.' }, 400);
  }
  ...
  try {
    const result = await withDatabaseClient(async (client) => {
      ...
      try {
        const corridorResult = await client.queryObject<Record<string, unknown>>(
          'select * from public.mobility_corridors where id = $1 for update',
          [corridorId],
        );
        const corridorRow = corridorResult.rows[0];
        if (!corridorRow) {
          throw new Error('Corridor not found.');
        }

        const corridor = mapMobilityCorridorRow(corridorRow);
        const currentProjection = projectCorridor(corridor);
        const available = type === 'seat'
          ? currentProjection.seats_available
          : currentProjection.cargo_available_kg;
        ...
        const updatedCorridor = advanceCorridorAfterBooking({
          corridor,
          type,
          quantity,
          timestamp,
        });
        const updatedProjection = projectCorridor(updatedCorridor);
        const unitPrice = type === 'seat'
          ? updatedProjection.dynamic_seat_price
          : updatedProjection.dynamic_cargo_price;
        const totalPrice = Number((unitPrice * quantity).toFixed(2));

        const bookingInsert = await client.queryObject<{ booking_id: string }>(
          `insert into public.mobility_bookings (
             corridor_id, user_id, type, quantity, unit_price, total_price, booking_timestamp, trace_id
           ) values ($1, $2, $3, $4, $5, $6, $7, $8)
           returning booking_id`,
          [corridorId, auth.canonicalUser.id, type, quantity, unitPrice, totalPrice, timestamp, traceId],
        );
        ...
        await client.queryObject(
          `update public.mobility_corridors
           set seats_booked = $2,
               cargo_booked_kg = $3,
               demand_index = $4,
               demand_history = $5::jsonb,
               price_history = $6::jsonb,
               updated_at = $7
           where id = $1`,
          [
            corridorId,
            updatedCorridor.seats_booked,
            updatedCorridor.cargo_booked_kg,
            updatedCorridor.demand_index,
            JSON.stringify(updatedCorridor.demand_history),
            JSON.stringify(updatedCorridor.price_history),
            updatedCorridor.updated_at,
          ],
        );

        const bookingCreated = createMobilityEventEnvelope({
          traceId,
        ...
        const capacityUpdated = createMobilityEventEnvelope({
          traceId,
          producer: 'CorridorService',
        ...
        const demandUpdated = createMobilityEventEnvelope({
          traceId,
          producer: 'DemandEngine',
          type: 'DemandUpdated',
          payload: {
            corridor_id: corridorId,
            demand_index: updatedCorridor.demand_index,
            previous_demand_index: corridor.demand_index,
            signal_weight: quantity,
            corridor: updatedCorridor,
            updated_at: updatedCorridor.updated_at,
          },
        });
        const priceRecalculated = createMobilityEventEnvelope({
          traceId,
          producer: 'PricingEngine',
          type: 'PriceRecalculated',
          payload: {
            corridor_id: corridorId,
            dynamic_seat_price: updatedProjection.dynamic_seat_price,
            dynamic_cargo_price: updatedProjection.dynamic_cargo_price,
            utilization: updatedProjection.utilization,
            demand_pressure: updatedProjection.demand_pressure,
            updated_at: updatedCorridor.updated_at,
          },
        });
        const corridorUpdated = createMobilityEventEnvelope({
          traceId,
        ...
        const events = [bookingCreated, capacityUpdated, demandUpdated, priceRecalculated, corridorUpdated];
        for (const event of events) {
          await client.queryObject(
            `insert into public.mobility_event_outbox (
               id, aggregate_type, aggregate_id, event_type, trace_id, payload, occurred_at
        ...
        return {
          bookingId,
          projection: updatedProjection,
          queuedEvents: events.map((event) => event.type),
        };
      } catch (error) {
    ...
    return json({
      ...
      status: 'accepted',
      trace_id: traceId,
      queued_events: result.queuedEvents,
      projection: result.projection,
    }, 202);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const status = message.includes('not found') ? 404 : message.includes('Not enough') ? 409 : 500;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (request.method === 'POST' && path === '/mobility-os/admin/apply-migrations') {
      return await handleApplyMobilityOsMigrations(request);
    }

    if (request.method === 'GET' && path === '/mobility-os/snapshot') {
      return await handleGetMobilitySnapshot(request);
    }

    if (request.method === 'POST' && path === '/mobility-os/booking/create') {
      return await handleMobilityBookingCreate(request);
    }

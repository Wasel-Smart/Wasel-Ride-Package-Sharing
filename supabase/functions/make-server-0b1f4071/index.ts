...
const CLIQ_API_BASE_URL = (Deno.env.get('CLIQ_API_BASE_URL') ?? Deno.env.get('JOPACC_API_BASE_URL') ?? '').replace(/\/$/, '');
...
const CLIQ_SECRET_KEY = Deno.env.get('CLIQ_SECRET_KEY') ?? Deno.env.get('JOPACC_SECRET_KEY') ?? '';
const CLIQ_WEBHOOK_SECRET = Deno.env.get('CLIQ_WEBHOOK_SECRET') ?? Deno.env.get('JOPACC_WEBHOOK_SECRET') ?? '';
...
function enforceRequestSecurity(request: Request, path: string): Response | null {
...
async function handleProfileRequest(request: Request, path: string) {
  ...
  if (request.method === 'PATCH') {
    ...
    if (isAdmin && typeof body.wallet_status === 'string') {
...
// The single Deno.serve dispatcher below iterates this table in source order.
// Each entry declares the HTTP method(s) it accepts and a path predicate; the
// first matching handler that returns a Response wins. Handlers may return
// `undefined` to fall through to the next entry (e.g. /trips and /bookings
// deep-link handlers). This replaces the previous ~310-line imperative if-chain
// so routing is scannable, order-deterministic and easy to extend. To split this
// into per-feature edge functions, group these handlers by domain and update
// supabase/functions/rate-limited-api/index.ts to forward each path prefix.

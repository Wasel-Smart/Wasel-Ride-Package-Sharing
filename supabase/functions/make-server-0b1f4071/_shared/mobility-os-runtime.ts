...
create index if not exists idx_event_outbox_unpublished
  on public.event_outbox(published_at)
...
drop policy if exists event_outbox_authenticated_select on public.event_outbox;
create policy event_outbox_authenticated_select

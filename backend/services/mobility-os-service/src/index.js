...
function buildMobilitySnapshot(corridors) {
    return {
...
function createApp() {
    ...
    `;
        res.json(buildMobilitySnapshot((corridors ?? [])));
    });
    ...
    `;
        if (from)
            query = sql `SELECT * FROM (${query}) WHERE origin_city ILIKE ${'%' + from + '%'}`;
        if (to)
            query = sql `SELECT * FROM (${query}) WHERE destination_city ILIKE ${'%' + to + '%'}`;
        if (seats)
            query = sql `SELECT * FROM (${query}) WHERE available_seats >= ${Number(seats)}`;
        if (date) {

...
class NotificationDispatcher {
    ...
    async registerDevice(registration) {
        ...
        await this.redis.hset(`device:${registration.userId}:${registration.deviceToken}`, 'platform', registration.platform, 'createdAt', registration.createdAt);
    }
...
function createApp() {
    ...
    app.post('/v1/communications/deliver', async (req, res) => {
      ...
      INSERT INTO communication_deliveries ${PostgresPool.connection(rows, 'user_id', 'notification_id', 'channel', 'delivery_status', 'destination', 'subject', 'payload', 'provider_name', 'queued_at', 'updated_at')}
    `;
    ...
    `;
        const count = (messages ?? []).filter((m) => !Array.isArray(m.read_by) || !m.read_by.includes(userId)).length;
        res.json({ count });
    });
    ...
    app.delete('/v1/devices/:token', async (req, res) => {
        ...
        await dispatcher.unregisterDevice(userId, token);

...
export class PaymentReconciliationService {
    ...
    async start() {
        ...
        this.unsubscribe = await eventBroker.subscribe('payments.authorized', this.handlePaymentAuthorization.bind(this), { groupName: 'payment-reconciliation-service', consumerName: `payment-worker-${process.env.HOSTNAME ?? 'local'}`, blockMs: 5000, count: 5 });
        this.ready = true;
    ...
    async shouldCapturePayment(auth) {
        if (auth.escrowStatus === 'released')
            return true;
        try {

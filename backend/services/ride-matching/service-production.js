...
class MatchingEngine {
    ...
    validateOrigin(origin) {
        if (!Number.isFinite(origin.lat) || origin.lat < -90 || origin.lat > 90)
            throw new ValidationError(`Invalid latitude: ${origin.lat}`);
        if (!Number.isFinite(origin.lng) || origin.lng < -180 || origin.lng > 180)
            throw new ValidationError(`Invalid longitude: ${origin.lng}`);
    }
    ...
    async executeMatch(rideId, candidates, origin) {
...
export class RideMatchingService {
    ...
    async start() {
        ...
        this.unsubscribe = await eventBroker.subscribe('rides.requested', this.handleRideRequest.bind(this), { groupName: 'ride-matching-service', consumerName: `matching-worker-${process.env.HOSTNAME ?? 'local'}`, blockMs: 5000, count: 10 });
        this.ready = true;
    ...
    async handleRideRequest(event) {
        ...
        try {
            ...
            if (!event.payload || typeof event.payload !== 'object')
                throw new ValidationError('Invalid ride request');
            const { rideId, origin, seats } = request;
            if (!rideId || !origin)
                throw new ValidationError('Missing rideId or origin');

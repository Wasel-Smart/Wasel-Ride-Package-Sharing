...
import { z } from 'zod';
import { ValidationError, ForbiddenError } from '@wasel/backend-shared/errors/app-errors';
...
export class TwoFactorService {
    ...
    async setup2FA(userId, deviceName) {
        const secret = authenticator.generateSecret();
        const backupCodes = this.generateBackupCodes();
        await this.redis.setex(`2fa:secret:${userId}`, 60 * 60 * 24 * 30, secret);
...
export function create2FARouter(redis) {
    const router = Router();

...
router.get('/rides/active', authenticate, requireRole(['admin', 'operator']), async (req, res) => {
    try {
...
router.get('/packages/pending', authenticate, requireRole(['admin', 'operator']), async (req, res) => {
    try {
...
router.get('/payments/reconciliation', authenticate, requireRole(['admin', 'operator']), async (req, res) => {
    try {
        ...
        const total = Number(countResult[0]?.total || 0);
        const payments = await db.unsafe(`SELECT p.*, u.full_name as user_name, t.origin_name, t.destination_name
       FROM payments p
...
router.get('/disputes', authenticate, requireRole(['admin', 'operator']), async (req, res) => {
    try {
        ...
        const total = Number(countResult[0]?.total || 0);
        const incidents = await db.unsafe(`SELECT si.*, u1.full_name as reporter_name, u2.full_name as reported_against_name
...
router.patch('/disputes/:id/resolve', authenticate, requireRole(['admin', 'operator']), async (req, res) => {
    try {
       ...
       WHERE id = $3 RETURNING *`, [resolution, userId, req.params.id]);
        if (!result[0]) {

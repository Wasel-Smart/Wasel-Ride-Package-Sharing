...
export class PackageRepository {
    ...
    async createPackage(input) {
        try {
            ...
            const result = await this.db.unsafe(`INSERT INTO packages (
        ...
        RETURNING *`, [
            ...
            throw new InternalError('Failed to create package', error);
        }
    }
    ...
    async findPackagesByStatus(status) {
        const result = await this.db.unsafe('SELECT * FROM packages WHERE status = $1 ORDER BY created_at DESC', [status]);
        return result;
    }
    async updatePackageStatus(id, status, carrierId) {
        try {
            ...
            throw new InternalError('Failed to update package status', error);
        }
    }
    async assignPackageToTrip(packageId, tripId, carrierId) {
        try {
            const tripResult = await this.db.unsafe('SELECT available_seats, allows_packages, package_capacity_kg FROM trips WHERE id = $1', [tripId]);
            if (!tripResult[0]) {
            ...
            const result = await this.db.unsafe(`UPDATE packages SET trip_id = $1, carrier_id = $2, status = 'matched', updated_at = NOW() WHERE id = $3 RETURNING *`, [tripId, carrierId, packageId]);
            return result[0];
        }
        catch (error) {
            ...
            throw new InternalError('Failed to assign package to trip', error);
        }
    }
    async findPackagesForRoute(originCity, destinationCity) {
        const result = await this.db.unsafe(`SELECT p.* FROM packages p
       ...
       ORDER BY p.created_at DESC`, [`%${originCity}%`, `%${destinationCity}%`]);
        return result;
    }
    async estimateDistance(from, to) {
        const result = await this.db.unsafe(`SELECT ST_Distance(
        ST_MakePoint($1, $2)::geography,
        ST_MakePoint($3, $4)::geography
      ) / 1000.0 as distance`, [from.lng, from.lat, to.lng, to.lat]);
        return Number(result[0]?.distance || 0);
    }
    calculateFee(size, distanceKm) {
        const baseFees = {
            small: 2.0,
            medium: 3.0,
            large: 5.0,
